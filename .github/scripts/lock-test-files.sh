...
if [[ "${#TEST_FILES[@]}" -eq 0 ]]; then
  echo "[lock-test-files] no test files found"
  exit 0
fi
...
for relative_path in "${TEST_FILES[@]}"; do
  ...
  if [[ -f "$absolute_path" ]]; then
    lock_file "$absolute_path"
    locked_count=$((locked_count + 1))
  fi
done

echo "[lock-test-files] locked $locked_count files"
echo "[lock-test-files] use docs/.github/scripts/unlock-test-files.sh to unlock"
