...
const activeDocuments = lifecycleRegistry.documents.filter(
  (entry) => entry.lifecycle === "active",
);
const activeDocumentsById = new Map(activeDocuments.map((entry) => [entry.id, entry]));
const activeRoutes = lifecycleRegistry.routes.filter((route) => route.lifecycle === "active");
const foundationDocuments = activeDocuments
...
function validateBootstrap(source, errors) {
  ...
  if (source.includes("Core 4") || source.includes("ACK/EVIDENCE")) {
    errors.push("content/AGENTS.md는 모든 세션의 Core 4 또는 ACK/EVIDENCE 선열람을 요구할 수 없습니다.");
  }
}
...
function validateRequestTypes(source, errors) {
  ...
  if (!sameArray(actualTypes, REQUEST_TYPES)) {
    errors.push(`요청 유형은 다음 순서의 폐쇄형 값이어야 합니다: ${REQUEST_TYPES.join(", ")}`);
  }
  for (const row of rows) {
    if (!row.action || !row.completion) {
      errors.push(`요청 유형의 동작과 종료 조건은 비어 있을 수 없습니다: ${row.type}`);
    }
  }
  requireText(
    section,
...
function validateScopeAndPermissions(source, errors) {
  ...
  const rows = parseTable(section, "권한", 2, "권한", errors).map(
    ([permission, condition]) => ({ condition, permission: stripInlineCode(permission) }),
  );
  const actualPermissions = rows.map((row) => row.permission);
  if (!sameArray(actualPermissions, PERMISSIONS)) {
    errors.push(`권한은 다음 순서의 폐쇄형 값이어야 합니다: ${PERMISSIONS.join(", ")}`);
  }
  for (const row of rows) {
    if (!row.condition) {
      errors.push(`권한 포함 조건은 비어 있을 수 없습니다: ${row.permission}`);
    }
  }

  const conditionByPermission = new Map(rows.map((row) => [row.permission, row.condition]));
  requireText(conditionByPermission.get("workspace 파일 변경") ?? "", ["변경·구현"], "파일 변경 권한", errors);
  requireText(conditionByPermission.get("외부 의존성") ?? "", ["사전 검토", "승인"], "외부 의존성 권한", errors);
  requireText(conditionByPermission.get("reviewer") ?? "", ["개인 또는 팀", "승인"], "reviewer 권한", errors);
  requireText(conditionByPermission.get("force push·삭제") ?? "", ["대상", "승인"], "파괴적 권한", errors);
  ...
  const routeContract = [...section.matchAll(/`(ROUTE:[^`]+)`/g)].map((match) => match[1]).at(0) ?? "";
...
function validateRoutes(source, routeExists, readRouteSource, errors) {
  ...
  const rows = parseTable(section, "적용 신호·단계", 2, "문서 라우팅", errors).map(
    ([signal, targetSource]) => ({ signal, targetSource }),
  );
  const expectedRows = activeRoutes.map(({ signal, targetSource }) => ({ signal, targetSource }));
  if (JSON.stringify(rows) !== JSON.stringify(expectedRows)) {
    errors.push("문서 라우팅 표가 document-lifecycle-registry.json의 active route와 다릅니다.");
  }

  const activeRouteTargetIds = new Set(activeRoutes.flatMap((route) => route.targets));
  for (const foundation of foundationDocuments) {
    if (!activeRouteTargetIds.has(foundation.id)) {
      errors.push(`기반 문서가 active route에서 참조되지 않습니다: content/${foundation.path}`);
    }
  }
  ...
  for (const route of activeRoutes) {
    ...
    for (const targetId of route.targets) {
      const target = activeDocumentsById.get(targetId);
      if (!target) {
        errors.push(`active route target 문서가 registry에 없습니다: ${targetId}`);
        continue;
      }
      ...
      for (const heading of target.requiredHeadings ?? []) {
        if (countExactHeadings(routeSource, heading.level, heading.title) !== 1) {
          errors.push(`라우팅 필수 Gate heading이 없습니다: ${relativePath}#${heading.title}`);
        }
      }
    }
...
function validateWorkspaceBootstrap(readmeSource, workspaceRootAgentsSource, errors) {
  ...
  requireText(
    readmeSource,
    ["docs/content/AGENTS.md", "ACK: BOOT@YYYY-MM-DD", "요청·단계별 필수 문서를 라우팅"],
    "README 새 세션 안내",
    errors,
  );
  if (
    workspaceRootAgentsSource !== null &&
    normalizeWhitespace(workspaceRootAgentsSource) !== normalizeWhitespace(readmeBootstrap)
  ) {
    errors.push("workspace root AGENTS.md bootstrap 계약이 README 예시와 다릅니다.");
  }
}
...
function extractSection(source, level, title) {
  ...
  return source.split("\n").slice(heading.endLine, next?.startLine ?? undefined).join("\n");
}
...
if (isMainModule) {
  ...
  const errors = validateAgentWorkflow({
    agentsSource: fs.readFileSync(path.join(docsRoot, "content", "AGENTS.md"), "utf8"),
    readmeSource: fs.readFileSync(path.join(docsRoot, "content", "README.md"), "utf8"),
