...
describe("release-preflight", () => {
  ...
  it("passes source closure when every service main uses the exact contract version", () => {
    const workspace = createWorkspace();
    writeContractSourceVersions(workspace, {
      api: "9.9.0",
      admin: "9.9.0",
      mobile: "9.9.0",
    });
    writeReleaseRecord(workspace.docsRoot, "v9.9.0", [
      ...
      "- `coupler-admin-web`: 태그 `N/A`, 커밋 `" + workspace.refs.admin + "`",
      "- `coupler-mobile-app`: Store `N/A`, 릴리스 태그 `N/A`, 커밋 `" + workspace.refs.mobile + "`, NextPush `N/A`",
    ], {
      status: "pending",
      releaseScopes: ["docs", "contracts-package"],
      extraRepoRefs: ["coupler-admin-web", "coupler-mobile-app"],
      apiContractCutover: pendingApiContractCutover(workspace.refs),
      pendingScope: "contract deployment",
    });
    commitAll(workspace.docsRoot);
  ...
  it("fails source closure when a consumer main uses a different contract version", () => {
    const workspace = createWorkspace();
    writeContractSourceVersions(workspace, {
      api: "9.9.0",
      admin: "9.9.0",
    ...
    assert.match(
      result.stdout,
      /coupler-mobile-app source main contract version must equal sourceClosure\.postMergeContract \(9\.9\.1 != 9\.9\.0\)/,
    );
  });
...
function writeContractSourceVersions(workspace, versions) {
  const apiPackageRoot = path.join(workspace.apiRoot, "packages", "contracts");
  fs.mkdirSync(apiPackageRoot, { recursive: true });
  fs.writeFileSync(
    path.join(apiPackageRoot, "package.json"),
    JSON.stringify({ name: "@coupler-developer/coupler-api-contracts", version: versions.api }, null, 2),
  );
  workspace.refs.api = commitAll(workspace.apiRoot);

  for (const [repoName, repoRoot] of [
    ["admin", workspace.adminRoot],
    ["mobile", workspace.mobileRoot],
  ]) {
    fs.writeFileSync(
      path.join(repoRoot, "package.json"),
      JSON.stringify({
        dependencies: {
          "@coupler-developer/coupler-api-contracts": versions[repoName],
        },
      }, null, 2),
    );
    workspace.refs[repoName] = commitAll(repoRoot);
  }
}

function pendingApiContractCutover(refs) {
  return {
    status: "pending",
    sourceClosure: {
    ...
    comparisonRefs: {
      "coupler-api": refs.api,
      "coupler-admin-web": refs.admin,
      "coupler-mobile-app": refs.mobile,
    },
