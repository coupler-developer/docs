...
const baseTestingStrategySource = readContent("content/policy/testing-strategy.md");
...
test("필수 section heading 누락을 거부한다", () => {
  const agentsSource = baseAgentsSource.replace("## 부트스트랩", "## 시작 안내");
...
test("새 세션 ACK 누락을 거부한다", () => {
  const agentsSource = baseAgentsSource.replace("ACK: BOOT@YYYY-MM-DD", "BOOT 확인");
...
test("권한 행 누락을 거부한다", () => {
  const agentsSource = baseAgentsSource.replace("| deploy | 별도 명시 요청 |\n", "");
  assert.match(validate({ agentsSource }).join("\n"), /권한은 다음 순서의 폐쇄형 값/);
});
...
test("라우팅 대상 파일 누락을 거부한다", () => {
  const errors = validate({
    routeExists: (relativePath) => relativePath !== "content/policy/payment-ops-policy.md",
  }).join("\n");
  assert.match(errors, /라우팅 문서가 존재하지 않습니다: content\/policy\/payment-ops-policy.md/);
});

test("라우팅 대상의 필수 Gate heading 누락을 거부한다", () => {
  const errors = validate({
    readRouteSource: (relativePath) =>
      relativePath === "content/policy/code-review-policy.md" ? "" : readContent(relativePath),
  }).join("\n");
...
test("testing strategy의 최종 후보 Gate heading 누락을 거부한다", () => {
  ...
  assert.match(validate({ testingStrategySource }).join("\n"), /로컬 최종 후보 검증/);
});

test("README bootstrap 예시 누락을 거부한다", () => {
  const readmeSource = baseReadmeSource.replace("# AGENTS (워크스페이스 전용)", "# Workspace");
  assert.match(validate({ readmeSource }).join("\n"), /bootstrap 예시를 찾을 수 없습니다/);
});

test("README 새 세션 안내 누락을 거부한다", () => {
  const readmeSource = baseReadmeSource.replace("ACK: BOOT@YYYY-MM-DD", "BOOT 확인");
