...
- 변경 작업: `<추가 | 수정 | 삭제 | 이동 | 개명 | 분리 | 통합 중 해당 작업 모두>`
- 대상:
- 비교 기준 ref:
- 직접 연결 문서: `<명시 링크 | 관련 문서 | 같은 도메인 규범 | 역방향 참조 | 영향받는 인덱스/nav/템플릿/검증 스크립트>`
- 제외:
...
판정 기준은 [문서 거버넌스 정책의 문서 생명주기 판정](../policy/document-governance-policy.md#문서-생명주기-판정)을
단일 기준으로 사용한다. Scope Gate에 기록한 변경 작업과 `N/A`가 아닌 행의 작업은 정확히 일치해야 한다.
적용한 변경 작업은 `No Findings` 또는 `Finding`, 적용하지 않은 작업은 근거 있는 `N/A`로 기록한다.
...
`policy` 변경의 동일 근거는 아래 `정책 Composition Gate` 표를 참조하고 중복 작성하지 않는다.
...
## 정책 Composition Gate (policy 문서의 생명주기 작업·리뷰 시)
...
| policy 문서의 생명주기 작업·리뷰에 대한 전체 정책·역방향 규범 참조, 책임/우선순위, 상태·단계별 Exit Gate, 전역 절 정합성, 삭제 시 책임 승계 증빙 있음 |  |  |
