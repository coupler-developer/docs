...
| 변경 작업 | 판정 | 근거 |
| --- | --- | --- |
| 추가: 기존 문서 검색, 추가 이유, 역할/SoT, 최초 진입 경로 |  |  |
| 수정: 전후 의미, 영향 절/정방향·역방향 참조, 결론 보존/기준 변경 |  |  |
| 삭제: 삭제 전 본문, 후속 문서/책임 종료, 참조·인덱스·검증 이관/제거 |  |  |
| 이동/개명: 의미 보존, 기존 경로 참조 0건 |  |  |
| 분리/통합: 변경 전 책임의 누락·중복 없는 이동 |  |  |
