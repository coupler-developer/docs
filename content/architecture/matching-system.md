...
신규 CMS 여성 수신 카드는 발송 로그인 스냅샷으로 3일 미로그인 여부를 판정하고, 낭비 카드 표시는 기존
Admin 수동 전체삭제의 기존 물리삭제 대상을 정확히 한정한다.
위 생명주기·관계 규칙은 일반 매칭의 보존 기준이다. 기존 Admin `낭비된 카드` 수동 전체삭제는 기존 동작의
예외로서 대상 `t_match` 원본만 물리 삭제하며, 이미 저장된 `t_match_log` 이벤트 이력은 그대로 남긴다.
