...
1. 쓰기 경계의 상태 동기화는 회원별 필수 인증 정책이 없을 때 현재 전담 클럽매니저 설정 또는 시스템
   기본값으로 정책을 생성한다.
2. 회원별 필수 인증 정책이 이미 있으면 현재 동기화 경로는 전담 클럽매니저 변경이나 설정 변경만으로 기존
   정책을 자동 덮어쓰지 않는다.
3. 회원이 심사 범위별 요청과 증거를 제출한다.
...
transaction에서 제거하고, 사진과 사진 심사 상태는 유지한다. DB 커밋 후에는 남은 참조가 없는 원본·썸네일
파일을 best-effort로 삭제한다. 세부 행위와 권한은 [회원 심사 단일 정책](../policy/member-review-policy.md)과
...
존재하는 상태에서는 현재 클럽매니저 준비 상태에 따라 동기화 guard가 저장을 차단할 수 있다. 정확한 발생
조건, 비적용 범위와 운영 Exit Gate는 [회원 심사 단일 정책](../policy/member-review-policy.md)의
