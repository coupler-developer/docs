...
- 초안, 삭제, 모집중, 모집마감, 확정, 종료, 취소 저장 상태와 저장 `CONFIRMED`·실효 `FINISHED` 시간 분기, 모집중·마감·확정 취소 진입 상태
- 신청, 승인, 승인 취소, 채팅방 탈퇴와 참여 가능한 모든 행사의 기준 여성 참여자 `tt@test.com`(`Toto`)·`dummy-female@coupler.dev`
- 행사 확정·종료·취소와 참여자 입장·승인 취소·탈퇴 시스템 메시지
- 전날 오후 1시 개방 전·후 확정 방, 저장 `CONFIRMED`·실효 `FINISHED` 방, 저장 `FINISHED` 방, 확정 후 취소 방에서
  두 기준 참여자의 일반 채팅, 호스트 채팅, 관리자 삭제 채팅, 비어 있음·설정됨 읽음 위치, 개방·종료 1초 전·정각·1초 후 파생 상태와 신청·승인 마감 전 시각
- 행사 생성·ready 상세 이미지·상태 action·신청/승인·상태별 취소/퇴장·채팅 멤버/메시지·후기/신고 action의 인과 순서와 기준 시각 이후 row 0건
- 프로필 열람 공개·비공개, 후기 성공·실패와 후기 완료 `APPROVED -> LEFT` 무메시지 전이, `APPROVED`·`LEFT` 후기 미작성, 두 기준 참여자가 신고자인 대기·처리·기각 신고와 미처리 filter
...
이 suite는 `t_group_meeting_*`와 신고 대상 namespace 합성 참가자의 `t_penalty_log`·`meet_block_date`만 대상으로 하며 `meeting-all`의 기존 2:2 데이터와 기존 QA 기준 회원의 회원 상태·패널티를 만들거나 reset하지 않는다.
