...
- 프로필 열람자와 대상이 같은 행사의 서로 다른 승인 참여자이며 실제 Mobile 조회 정책을 통과하는지 검증
...
2026-07-19 `coupler-admin-web` `origin/main` `97aebcd60bb15e74519dbdb1c85f6d2bba190514`의 `src/config/page-route.tsx`를 기준으로 component가 연결된 route는 총 50개다.

- sidebar가 사용하는 `getFilteredRouteList(true, false)`의 `Menu` leaf 47개는 모두 `data-surface`다.
...
따라서 현재 모집단은 component route 50개, 데이터 화면 48개, `non-data` 2개다. 아래 표는 47개 sidebar 데이터 탭을 설명하며, 이후 실제 모집단은 typed route descriptor와 exact-set coverage test가 자동 판정한다.
...
47개 sidebar 탭과 회원 상세 1개, 총 48개 데이터 화면은 `live-only` 대상이 아니다. 각 화면은 `scenario-backed` 또는 `reference-backed`로 분류되고 API 결과와 실제 렌더 결과를 가져야 한다.

기존 2:2 그룹미팅 5개 route는 N:N 그룹미팅 전환에 따라 CMS 메뉴와 component route에서 제거됐으므로 현재 Admin 화면 모집단에 포함하지 않는다. `meeting-all` suite는 레거시 데이터 불변식과 API smoke를 위해 유지하지만 Admin route coverage는 대시보드 `cms-all` 집계에서만 해당 scenario의 존재를 확인한다.
