...
기존 알림 흐름과 그룹미팅 77~85의 API 타입은 현행이다. 별도 Mobile 소비자 정렬과 운영 증빙은
[기술 부채 정리](../technical-debt/technical-debt.md)의 `그룹미팅 소비자 정렬 및 출시 통합 미완료`에서 추적한다.
...
연결한다. 확정 80은 과거 알림 재진입 호환용으로 소비하되 신규 확정에서는 발송하지 않는다. 소비자 정렬과 운영 전환 상태는
[기술 부채 정리](../technical-debt/technical-debt.md)의 `그룹미팅 소비자 정렬 및 출시 통합 미완료`에서
...
처리값으로 기록해 다음 cron의 중복 대상에서 제외한다. 행사 단위 marker는 cron batch 요약으로만 사용한다.
83은 행사 시작 +24시간 뒤 현재
