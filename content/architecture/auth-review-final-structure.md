...
- legacy endpoint(`POST /member/addAuth`, `GET /admin/member/pending/list`)를 제거하고 단일 계약으로 고정한다.
...
### 8.3 signup 단계 심사 (queue_type 비노출)

- signup 단계 판정은 서버 내부 상태 전이 규칙(`resolvePendingScope`)으로만 처리한다.
- Admin `review-queue` 응답의 `queue_type`은 `profile-edit`, `review-item-reapply-queue` 2개만 허용한다.
- 즉, `semi/full/intro-*` 타입을 Admin 조회 API 계약으로 다시 노출하지 않는다.
...
- `queue_type` 허용값: `profile-edit` | `review-item-reapply-queue`
...
### 10.4 레거시 호환 제거 (완료 상태)

- 제거 완료:
  - `POST /member/addAuth`
  - `GET /admin/member/pending/list`
- Mobile 제출 경로는 `POST /member/review/auth/submit`만 사용한다.
- Admin 큐 조회 경로는 `GET /admin/member/review-queue`만 사용한다.
...
- `POST /member/addAuth` 제거 완료
- `signup`/`설정` 인증 저장 경로를 공통 UseCase(`submitAuthReviewUseCase`)로 고정
- Admin 조회는 `review-queue`만 사용하고 `pending/list`는 제거 완료
