...
- 삭제 이력 도입 전 게시글의 `-1`은 작성자 삭제와 CMS 삭제를 구분할 근거가 없고 앱 목록에서도 모두
  제외되던 값이다. 현행 데이터에서는 과거 콘텐츠가 다시 노출되지 않도록 이 게시글을
  `ADMIN_FORCE_DELETED(-3)`로 보존한다.
- 삭제 이력 도입 뒤의 작성자 게시글 삭제는 `AUTHOR_DELETED(-1)`을 사용한다.
- 삭제 이력 도입 전 댓글 `-1`은 앱에서 tombstone으로 노출되던 작성자 삭제 상태로 해석한다.
- 삭제 이력 도입 전 행위 시각은 원천 데이터에 없으므로 생성일이나 migration 실행 시각으로 합성하지 않는다.
  이 경우 CMS는 삭제 상태와 함께 `기록 없음 (삭제 이력 기능 적용 전 데이터)`로 구분한다.
...
| --- | --- |
| 카테고리 | `category = 12` |
| 노출 범위 | `lounge_tab = MANAGER_ONLY`; 작성자와 같은 전담 클럽매니저(`CHARGE`) 배정을 가진 회원 |
...
- API는 `MANAGER_ONLY` 게시글의 작성·목록·상세 접근에서 전담 클럽매니저 배정을 확인하고, 조회 대상과
  조회 회원의 매니저가 다르면 접근을 거부한다.
...
- `coupler-api`가 공통 설정의 라운지 카테고리 코드 12와 기존 `MANAGER_ONLY` 접근 통제를 제공하고,
...
| 노출 범위 | Mobile은 `MANAGER_ONLY`로 등록하고 API는 같은 전담 클럽매니저 배정 회원에게만 작성·조회 접근을 허용한다. | Mobile 등록 payload와 API의 매니저 배정 있음·없음·다른 매니저 접근 테스트 |
...
| 사진 불변조건 | 사진은 선택 사항이며 최대 3장이다. 0장이면 업로드를 생략하고 선택한 사진이 있으면 업로드 결과를 등록 payload에 반영한다. | `0`장 업로드 생략·등록, `1`, `3`장 업로드·등록과 `4`장 선택 제한 경계 테스트 |
| 개인정보 확인 | 자동 생성한 프로필 초안과 `mini_public = N`의 본문 노출 의미를 등록 전에 회원이 확인·수정할 수 있다. | 초안 수정·삭제와 미니프로필 비공개 게시 수동 시나리오 |
| 하위 호환 변경 | 기존 앱에서 같은 전담 클럽매니저 범위의 코드 12 게시글 목록·상세를 읽을 수 있고 신규 Mobile에서 작성·조회할 수 있다. | 기존 앱 `MANAGER_ONLY` 목록·상세와 신규 앱 작성·조회 시나리오 |
