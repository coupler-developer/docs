...
| --- | --- | --- | --- | --- | --- | --- |
| `analytics.login-statistic` | 로그인 통계 | root | projection | 일자·성별·시간대별 로그인 집계 | 내부 | 원천 데이터에서 재생성 가능하며 집계 보관 정책 적용 |

### 관계

| 출발 논리 ID | 관계 유형 | 도착 논리 ID | 카디널리티 | 소유·삭제 규칙 |
| --- | --- | --- | --- | --- |
| `analytics.login-statistic` | derives-from | `member.member` | N:M | 개인 식별정보 없이 집계하며 원천 회원의 현재 상태를 대체하지 않음 |
