...
| N:N 그룹미팅 채팅 | `group-meeting.event` | 호스트와 승인 참가자 | 독립 N:N 그룹미팅 채팅 |

## Mobile 전역 unread 빨간점

### 의미와 snapshot

Mobile 채팅 하단 탭의 빨간점은 현재 회원이 접근 가능한 지원 채팅에서 DB 기준 unread 메시지가 하나라도 있는지만
나타낸다. VIP, 기존 2:2 미팅, `t_match.male_badge`, `t_match.female_badge`, 메시지가 아닌 상태 알림은 이 값에
영향을 주지 않는다.

| 원천 | unread 조건 |
| --- | --- |
| 큐레이터 | 현재 회원의 `t_concierge.is_send='N' AND status='N'`; 가입 승인 자동 환영 메시지 포함 |
| 1:1 매칭 | `GET /app/chat/chatList`의 성별별 현재 노출 조건을 만족하는 매칭의 상대방 메시지 `status='N'` |
| N:N 그룹미팅 | 호스트 또는 현재 `APPROVED` 참가자로 접근 가능한 전체 방의 정상 메시지 중 내 history boundary 이후, 내 read watermark 이후인 상대 `USER` 또는 `SYSTEM` 메시지 |

`GET /app/chat/unread`는 표준 성공 envelope의 `data`로 `{ "has_unread": boolean }`만 반환한다. 서버는 세 원천을
한 SQL statement의 세 `EXISTS`로 계산하며 그룹 채팅 목록의 첫 20개 pagination을 재사용하지 않는다. 1:1 노출
조건은 채팅 목록과 공용 predicate를 사용해 상태·만료·후기 경계가 서로 달라지지 않게 한다. 조회 전용 애플리케이션
코드이므로 별도 DB migration은 필요하지 않는다.

그룹 unread는 `status=1`, `id >= history_visible_from_message_id`,
`id > COALESCE(last_read_message_id, 0)`,
`message_kind='SYSTEM' OR sender_chat_member_id <> self`를 모두 적용한다. 따라서 21번째 이후 방과 시스템 메시지도
포함하고, 승인 전 history와 Admin 삭제 메시지는 제외한다.

### realtime invalidation

전역 unread 이벤트는 truth를 운반하지 않는다. 서버는 다음 payload 없는 envelope만 회원 identity room에 보낸다.

```json
{
  "type": "chat:unread:invalidated"
}
```

별도 전역 revision 없이 boolean을 이벤트에 넣으면 새 메시지 뒤 늦게 도착한 오래된 `false`가 `true`를 덮을 수
있다. 따라서 Mobile은 이벤트를 DB 값이 달라졌을 수 있다는 신호로만 사용하고 snapshot을 다시 조회한다.

| 변경 | invalidation 대상 |
| --- | --- |
| 큐레이터 Admin 전송·가입 승인 자동 환영 메시지 | 해당 회원 |
| 큐레이터 회원 읽음 성공 | 읽은 회원 |
| 1:1 메시지 생성 | 그 메시지를 unread로 보는 상대 회원 |
| 1:1 읽음 성공 | 읽은 회원 |
| 1:1 노출 상태·만료·후기 상태 변경·삭제 | 변경 전·후 양쪽 회원 |
| 그룹 `USER`·`SYSTEM` 메시지 | 변경 뒤 현재 구성원; `USER` 발신자의 watermark 전진 때문에 발신자 포함 |
| 그룹 읽음 성공 | 읽은 회원 |
| 그룹 승인·취소·퇴장 | 변경 전·후 구성원의 합집합 |
| 그룹 행사 모집 마감·취소·종료, Admin 메시지 삭제 | 영향받는 현재 구성원 |

업무 transaction이 commit된 뒤에만 best-effort로 발행한다. 수신자 조회나 발행 실패는 이미 성공한 업무 결과를
실패로 바꾸지 않으며, 다음 복구 snapshot이 원본을 확정한다.

### Mobile 상태와 복구

`ChatUnreadState.hasUnread`는 영속 저장하지 않는 로그인 세션 메모리 캐시다. 로그인 세션 시작, foreground 복귀,
`realtime:ready`, `chat:unread:invalidated`, 큐레이터·1:1·그룹 읽음 성공 직후에 snapshot을 요청한다. foreground
동안에는 5분마다 boolean 한 건을 reconciliation하며 background에서는 조회하지 않는다. 이 주기는 best-effort
이벤트 유실과 `CHAT_3_DAYS_OVER` 방의 `match_expire_date` 도래 같은 DB write 없는 시간 경계를 복구한다.

동시 신호는 한 요청으로 합친다. 조회 중 신호가 오면 그 응답을 적용하지 않고 완료 뒤 trailing 조회를 한 번만
실행한다. 같은 세션에서는 한 번에 한 요청만 허용하며, logout·token 교체는 세대를 올리고 이전 요청을 abort한 뒤
...
| 서버 이벤트 | `realtime:ready`, `realtime:error`, `concierge:message`, `concierge:read:ack`, `concierge:read:updated`, 회원 endpoint의 `chat:unread:invalidated` |
...
- 최초 모집 마감은 승인 인원과 무관하게 행사당 채팅 principal과 현재 승인 구성원을 한 번만 초기화한다.
  이후 OPEN·CONFIRMED 상태를 왕복해도 방·구성원·메시지를 삭제하거나 다시 만들지 않는다.
- 실제 접근은 최신 행사 일시의 달력상 전날 KST 13시에 열리고 행사 시작 +24시간에 읽기 전용으로 바뀐다.
  행사 일시 변경 시 두 경계를 다시 계산한다.
- 읽기 전용 방은 메시지와 미작성 후기 문맥만 보존한다. 구성원 요약 아바타는 과거 메시지 식별을 위해 남기지만
  다른 구성원의 전체 프로필 열람과 그룹미팅 출처 매칭카드 사전 검사·전달은 허용하지 않는다.
