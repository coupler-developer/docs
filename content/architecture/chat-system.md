...
| GET | `/app/chat/list` | 기존 Mobile offset 소비자 호환용 목록과 관리자 발신 읽음 처리(Deprecated) |
...
| GET | `/admin/member/concierge/chat_list` | 기존 Admin 배열 소비자 호환용 전체 목록과 회원 발신 읽음 처리(Deprecated) |
| POST | `/admin/member/concierge/send` | Admin 멱등 메시지 저장 후 canonical 메시지 반환·신규 이벤트·FCM 발행 |

신규 cursor `GET /app/chat/messages`, `GET /admin/member/concierge/messages`는 읽기 전용이다. 조회 과정에서
`status`를 바꾸지 않으며 신규 소비자의 읽음 변경은 아래 WebSocket 명령으로만 수행한다. 단, 구 Mobile bundle과
Admin artifact는 WebSocket 읽음 명령을 보내지 않으므로 Deprecated `/app/chat/list`,
`/admin/member/concierge/chat_list`에만 기존 방향별 읽음 변경을 호환 동작으로 유지한다. 실제 변경 행이 있으면
혼합 버전의 신규 소비자도 즉시 반영하도록 같은 읽음 경계를 `concierge:read:updated`로 발행한다. 이 경계는 별도
최신 ID 조회가 아니라 실제 응답에 포함된 상대방 발신 메시지의 최대 ID로 계산해, 목록 조회 직후 도착한 미전달
메시지를 읽음 처리하지 않는다. Admin Deprecated endpoint는 기존 artifact의 전체 배열 응답 범위도 그대로 유지한다.
mandatory 전환과 legacy endpoint traffic 0건이 확인된 뒤 별도 cutover에서 endpoint와 함께 제거한다.

신규 두 `.../messages` endpoint는 최신순으로 최대 `limit`건을 반환하고, 더 오래된 메시지가 있으면 마지막 반환
...
과거 페이지 경계가 밀리지 않는다. 큰 `OFFSET`과 전체 `COUNT`는 사용하지 않는다. 기존 두 목록 endpoint는
배포 호환 경계일 뿐 신규 소비 경계가 아니며, 제거 조건은 [기술 부채 정리](../technical-debt/technical-debt.md)의
`큐레이터 offset/배열 목록 endpoint 제거 대기`에서 추적한다.
...
두 `POST .../send`의 최종 계약은 printable ASCII 64자 이하의 `client_message_id`를 필수로 받는다. 현재 호환
배포에서는 기존 Admin artifact와 Mobile bundle이 필드를 생략한 경우에만 서버가 요청마다 `compat-` 키를 생성하고
`[concierge-client-message-id] compatibility fallback used` 로그에 `surface`, `actor_id`, `member_id`를 남긴다.
명시한 값이 빈 문자열·형식 오류이면 호환 처리하지 않고 거부한다. 요청마다 새 키를 만드는 기존 bundle 요청은 HTTP
재시도 멱등성을 보장하지 않으며, 새 Admin·Mobile만 클라이언트가 같은 키를 유지해 안전하게 재시도한다.
...
호환 경로는 [기술 부채 정리](../technical-debt/technical-debt.md)의
`큐레이터 client_message_id 호환 경로 제거 대기`에서 추적한다. 새 Admin artifact 배포, Android·iOS NextPush
`Production` mandatory 적용, 이전 bundle 강제 업데이트 차단, 호환 로그가 양 surface에서 24시간 연속 0건인
근거가 모두 확보되면 별도 cutover PR에서 helper를 삭제하고 Swagger 필드를 다시 required로 바꾼다. 이번 호환
배포에서 제거하지 않는다.
...
- 배포 순서는 `t_concierge` sender/idempotency nullable expand와 DB gate 검증, contracts package `0.1.16`
  ...
  활성 세션의 전송을 깨지 않는다. JS/TypeScript-only 변경이라 Store 재심사는 하지 않는다. 로그인한 Super 또는
  담당 관리자와 테스트 회원으로 메시지 도착·멱등 재시도·안 읽은 수·양방향 읽음 표시를 확인한다.
- mandatory 적용과 강제 업데이트 차단 뒤 호환 로그를 surface별로 24시간 관측한다. 0건과 exact package 정렬을
  릴리즈 기록에 남긴 뒤에만 별도 cutover에서 `client_message_id`를 필수화한다.
- 문제가 있으면 Admin·Mobile 실시간 소비 코드를 먼저 롤백한 뒤 API WebSocket 기능을 롤백한다. 기존 HTTP 응답
  계약, nullable expand DB 스키마, API의 누락 필드 호환 경로는 유지한다. 구 Mobile bundle을 다시 활성화해야 하면
  mandatory 설정과 호환 API 기준점을 함께 복구하고 cutover를 진행하지 않는다.
