...
- 프로필 영상의 canonical 저장 경로는 `uploads/video/`로 시작하는 상대경로이고, 프로필 버전이 그 경로를
  참조한다. 레거시 회원 영상 컬럼을 별도 저장소나 fallback으로 사용하지 않는다.
- Mobile 정회원 본인과 Super Admin의 명시적 삭제는 현재 프로필과 최신 심사 중 프로필의 영상 참조를
  DB transaction에서 제거한다. 두 버전의 파일과 같은 경로를 가리키는 해당 회원의 과거 참조도 같이
  제거하고, 관계없는 과거 영상 참조는 유지한다.
- 같은 DB transaction에서 삭제 후보가 다른 프로필 버전에서 아직 참조되는지 확인한다. 참조가 남아 있으면
  파일을 유지하고, 없으면 원본 영상과 같은 stem의 `.jpg` 썸네일을 파일시스템에서 먼저 삭제한 뒤 DB를
  커밋한다. 원본이나 썸네일을 덮어쓰거나 변형·이동하지 않는다.
- 원본과 썸네일이 이미 없는 경우는 삭제 성공으로 본다. 하나라도 삭제에 실패하면 DB 변경을 rollback하고
  API 실패를 반환한다. 주기 scanner·worker·outbox·cron 재시도는 두지 않는다.
- DB와 파일시스템은 하나의 원자적 transaction이 아니다. 순차 삭제 중 일부 파일만 삭제되고 나머지가
  실패하거나, 파일 삭제 뒤 DB 커밋 실패·프로세스 종료가 발생하면 DB 참조가 남고 일부 파일은 없을 수
  있다. 이미 삭제된 파일은 rollback으로 복원하지 않으며, 다음 요청에서 없는 파일을 성공 처리해 수렴한다.

삭제 대상 프로필 버전, 사진 심사 보존과 권한은 [회원 심사 단일 정책](../policy/member-review-policy.md)을
따른다.
