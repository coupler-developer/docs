...
- DB 커밋 후 삭제 후보가 다른 프로필 버전에서 아직 참조되는지 다시 확인한다. 참조가 남아 있으면 파일을
  유지하고, 없으면 원본 영상과 같은 stem의 `.jpg` 썸네일을 파일시스템에서 삭제한다. 원본이나 썸네일을
  덮어쓰거나 변형·이동하지 않는다.
- 파일 삭제 실패는 로그로 남기되 이미 커밋된 DB 삭제를 rollback하지 않는다. 현재 구조는 이 후처리를
  위한 주기 scanner·worker·outbox를 두지 않는다.
