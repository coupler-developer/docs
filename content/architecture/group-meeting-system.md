...
채팅방·메시지 HTTP snapshot이다. 상세 전송·병합 계약은 [채팅 시스템](chat-system.md)의 N:N 절을 따른다.
...
- Super Admin은 `manager_user_id`와 모바일 회원 이메일을 정확히 조회해 최초 호스트 연결을 만든다. 행사
  생성은 `manager_user_id`만 받고 API가 내부 연결을 해석하며, 연결 또는 공통 이용 자격을 충족하는 모바일 계정이 없으면
...
- 모임 만들기와 기존 모임 수정은 별도 중첩 모달 없이 같은 `미팅정보` 폼을 사용한다. 일반 클럽매니저의
  ...
  주최자를 선택한다. 연결된 공통 이용 자격 충족 모바일 계정이 없으면 호스트 연결 관리가 필요하다는 안내를 표시한다.
