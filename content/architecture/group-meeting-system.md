...
1차 범위는 `행사 생성 -> 모집 시작 -> 모집 중·확정 운영 -> 신청·Admin 승인/확정 취소 -> 그룹 채팅 -> 종료 -> 후기`다.
...
- Admin은 DRAFT에서 행사 정보 수정 API의 모든 필드를 변경할 수 있다. OPEN·CONFIRMED에서는 참가비·
...
??? info "정확한 값과 조건 보기"
    ...
    | `group-meeting.event` | 그룹미팅 행사 | root | entity | state | 모집·신청 접수·확정·종료와 공개 행사 정보 | 민감 | 삭제·취소·종료 상태로 보존, 공개 이미지는 정책에 따라 정리 |
...
신청 접수 여부는 행사 상태와 별도로 `application_close_at`과 명시적 재개 marker에서 계산한다. 기한이 지난
OPEN에 `/open` 명령을 실행하면 상태 값은 1로 유지하면서 `APPLICATION_REOPENED` 감사 이력과 marker를 한 번
기록한다. CONFIRMED를 OPEN으로 되돌리는 명령은 기존 `EVENT_OPENED` 이력을 사용하고 초기화된 채팅을 보존한다.
...
시작 시점의 `application_is_open`이 true일 때만 변경할 수 있고 CONFIRMED 또는 신청 접수가 닫힌 OPEN에서는
현재 값으로 고정한다. 마감 시각 변경과 잠긴 모집 조건 변경을 한 요청으로 함께 보내도 요청 시작 시점 판정을
...
참여 승인과 확정 취소는 행사 OPEN·CONFIRMED에서만 허용한다. 채팅 초기화 뒤 승인하면 chat member와
