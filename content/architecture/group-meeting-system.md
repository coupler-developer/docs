...
1차 범위는 `행사 생성 -> 모집 -> 신청 -> Admin 승인/확정 취소/거절 -> 참가 확정 -> 그룹 채팅 -> 종료 -> 후기`다.

- 포함: 행사 목록/상세/내 모임, 긴 상세 이미지, 성별 정원, 신청/승인/확정 취소/거절, 미니프로필 열람,
...
| 3 | `t_group_meeting_event_mood` | 행사 분위기 다중값 정규화 |
...
- Admin의 참여 확정 취소는 application을 `APPROVED -> APPLIED`로 되돌리고 `approved_at`을 null로
  갱신한다. 별도 취소 상태·시각 컬럼·이력 테이블은 만들지 않고 취소 actor/reason/time은 action log에
  보존한다.
...
| 79 | `GROUP_MEETING_APPLICATION_APPROVAL_REVOKED` | 신청자 / Admin 확정 취소 | `alarm_event` | event ID |
| 80 | `GROUP_MEETING_APPLICATION_REJECTED` | 신청자 / Admin 거절 | `alarm_event` | event ID |
...
- 운영 `t_alarm.type`은 signed `TINYINT`이고 현재 최대 타입은 76이므로 77~85는 범위 안의 미사용 값이다.
  ...
  연속 구간으로 옮긴다.
- 기존 `sendFCMPush()`는 현재 `alarm_chat`, `alarm_match`만 일부 타입에 적용하므로 77~83/85에는
  `alarm_event`, 84에는 `alarm_chat`을 적용하는 서버 분기와 테스트를 추가한다. 설정이 N이면 FCM과
  `t_alarm` 저장을 모두 건너뛴다.
- 서버 push content, Mobile 상수와 foreground/background/open 라우팅, 알림 목록 target 라우팅을 함께
  추가한다. 현재 앱은 미정의 타입에서 crash는 나지 않지만 탭 라우팅을 하지 않으므로, 77~85를 인식하는
...
| --- | --- | --- | --- | --- |
| event_id | INT | N | - | 행사 |
| mood_code | TINYINT UNSIGNED | N | - | 신규 `GROUP_MEETING_MOOD` 코드 1~6 |

- `PRIMARY KEY(event_id, mood_code)`
- 코드값의 의미는 기존 분위기 1~6과 같게 유지하되 신규 서버 enum은 `GROUP_MEETING_MOOD`로 선언한다.
  기존 `MEET_MOOD` 상수를 신규 도메인 계약으로 재사용하지 않는다.

### 4. `t_group_meeting_event_detail_image_version`
...
### 5. `t_group_meeting_event_detail_image_slice`
...
| approved_at | DATETIME | Y | NULL | 현재 승인 시각, 확정 취소 시 NULL, 재승인 시 갱신 |
...
| APPLICATION | `APPLICATION_APPROVED`, `APPLICATION_APPROVAL_REVOKED`, `APPLICATION_REJECTED`, `APPLICATION_CANCELED`, `APPLICATION_LEFT` |
...
OR (action = 'APPLICATION_APPROVAL_REVOKED' AND from_status = 1 AND to_status = 0)
...
- action log의 reason에는 연락처·프로필 원문·인증정보를 넣지 않고 승인/확정 취소/거절/삭제의 최소 운영
  사유만 남긴다.
...
- 허용 전이: `APPLIED -> APPROVED | REJECTED | CANCELED`,
...
1. Admin 거절은 APPLIED application을 REJECTED로 바꾸고 action log를 같은 transaction에 저장한 뒤
...
4. 행사 종료는 event를 FINISHED로 바꾸고 action log를 같은 transaction에 저장한 뒤 commit 성공 시
   APPROVED/LEFT 참가자에게 85 타입을 보낸다.
...
| --- | --- |
| `GroupMeetingEventListItem` | event + host/member 요약 + mood 목록 + application summary VIEW + 내 application status |
...
- 행사 CONFIRMED 전 참여 확정 취소는 application을 APPLIED로 되돌리고 approved_at을 null로 만들며
  version과 `APPLICATION_APPROVAL_REVOKED` action log를 함께 반영한다. CONFIRMED 이후에는 실패한다.
...
- 신고 처리, 메시지 삭제, 승인/확정 취소/거절, 행사 취소는 action log 없이 완료되지 않는다.
