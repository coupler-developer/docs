...
- 포함: 행사 목록/상세/내 모임, 긴 상세 이미지, 성별 정원, 신청/승인/확정 취소, 미니프로필 열람,
  그룹 채팅/unread, 채팅 구성원 신고, 후기와 Key 보상, 푸시, 운영 감사
- 제외: 현장 체크인, 좌석/회전 라운드, 라운드별 호감 선택, 전역 회원 패널티, 외부 결제/정산
- 호스트는 참가 정원에서 제외한다.
- 남녀 정원은 각각 2명 이상이며 합계는 최대 30명으로 고정한다.
...
- 모임 확정은 남녀 승인 인원이 각각 2명 이상이면 Admin이 수행한다. 남은 APPLIED는 별도 미선정 상태로
  바꾸지 않고 일반 화면에서는 미확정으로 표시한다. 다만 CONFIRMED 이후 개인 확정 취소가 발생하면 Admin은
  남은 APPLIED를 대체 참가자로 승인해 기존 채팅방에 합류시킬 수 있다.
...
- Admin CMS의 운영 목적 프로필 조회는 무료이며 감사 로그를 남긴다. Mobile 호스트도 자신이 개설한 행사의
  ...
  때만 Key를 차감한다.
- Mobile 호스트는 신청 row가 한 건도 생기지 않은 DRAFT/OPEN/CLOSED 행사만 수정하거나 삭제할 수 있다.
  DRAFT 삭제는 DELETED, OPEN/CLOSED 삭제는 CANCELED로 보존한다. 신청 row가 한 건이라도 생긴 뒤에는
  신청 상태와 관계없이 Mobile 수정/삭제를 금지하고 CMS 운영 흐름만 사용한다.
- CONFIRMED 행사는 `event_at + 24시간`이 지나면 서버 cron이 FINISHED로 자동 전이한다. Admin 수동 종료
  API는 두지 않는다.
...
- 그룹미팅 migration은 기존 16/18 row를 변경하지 않고 공유한다. 25번은
  `id=25, value='5', title='그룹미팅(Host) 후기 보상', content='후기작성 시 보상', admin='admin',
...
- 기존 Admin 설정 저장 API는 16/18번에 `-2147483647~-1` canonical 정수, 25번에
  `1~2147483647` canonical 정수만 허용하며 공백·지수 표기·소수·잘못된 부호·범위 밖 값은 저장 전에
  거절한다.
...
| 10 | `t_group_meeting_report` | 같은 행사 채팅 구성원 간 회원 신고 |
...
- report의 reporter/target member ID도 신고 당시 감사 snapshot으로 보존하며 회원 FK를 두지 않는다.
  target은 필수이며 접수 시 두 회원 모두 해당 행사의 현재 채팅 구성원인지 server transaction에서 검증한다.
  행사글 자체 신고는 만들지 않고 회원 삭제 뒤에는 원천 회원과 다시 연결하지 않는다.
...
- 행사 생성의 호스트 선택은 `t_group_meeting_host -> t_member`를 조회해 회원 이메일 exact match 또는
  nickname 부분 일치로 찾는다. 이메일을 event/host에 중복 저장하거나 별도 manager-assignment 테이블을
  사용하지 않는다.
...
| detail_text | TEXT | Y | NULL | 이미지 외 안내문, 앞뒤 공백 제거 후 UTF-8 65,535 bytes 이하 |
...
| fee_amount | INT UNSIGNED | N | 0 | 화면 표시 회비, 0=없음, API 허용 범위 0~4294967295 |
| photo_public | TINYINT | N | 0 | 0=미니프로필, 1=사진 공개 |
| male_capacity | TINYINT UNSIGNED | N | - | 남성 정원, 2 이상 |
| female_capacity | TINYINT UNSIGNED | N | - | 여성 정원, 2 이상, 남녀 합계 30 이하 |
...
- `INDEX(status, created_at, id)`, `INDEX(status, application_close_at)`,
  `INDEX(status, event_at, id)`, `INDEX(host_id, status, created_at, id)`
...
- Admin은 OPEN 이후 핵심 모집 조건을 변경하지 않는다. Mobile 호스트는 DRAFT/OPEN/CLOSED라도 application
  row가 0건일 때만 전체 행사 필드를 수정할 수 있으며, 최초 신청이 생긴 뒤에는 수정/삭제할 수 없다.
...
- 신규 upload는 `created_by_admin_id`를 반드시 채운다. Admin 삭제 transaction은 행사에 연결되지 않은
  version과 slice metadata를 먼저 삭제하고 commit 뒤 원본/slice 파일을 정리한다. 행사 이력에 연결된
  version만 보존하며, 이때 creator FK를 `ON DELETE SET NULL`로 바꾸기 위해 nullable을 허용한다.
...
- 현재 API 프로세스의 직렬 worker queue에 남아 있는 version은 상태 조회가 만료시키지 않는다. worker 프로세스는
  작업 시작 후 5분에 종료하며, API 재시작으로 메모리 queue가 유실된 경우에만 pending 30분, processing 6분을
  지난 version을 처리 중단 실패로 정리한다.
...
- 신규 승인과 확정 취소는 행사 OPEN/CLOSED/CONFIRMED에서 허용한다. CONFIRMED에서 APPLIED를 승인하면
  같은 transaction에 chat member를 생성해 기존 채팅방에 참여시키고, APPROVED를 CANCELED로 바꾸면 기존
  chat member는 이력으로 보존하되 해당 회원의 채팅 읽기·쓰기 권한을 즉시 차단한다.
...
- 호스트 row는 `host_id`만, 참가자 row는 `application_id`만 채운다. 호스트 표시는
  ...
  참가자 row는 생성 시 APPROVED application을 참조하고 이후 application이 CANCELED/LEFT가 되거나 후기
  작성이 완료돼도 메시지 표시 이력 때문에 보존한다. 참가자 회원과 표시 별칭은 application에서 조회한다.
- HOST의 송신 자격은 event.host_id의 host.member_id가 요청 회원과 같고 유효한 회원 상태인지, 참가자의
  송신 자격은 현재 APPROVED application이고 후기를 작성하지 않았는지로 판정한다. CANCELED/LEFT 또는
  후기 작성 완료 application의 chat member는 읽기·쓰기 자격 없이 이력으로만 남는다.
...
| target_member_id | INT | N | - | 같은 행사 채팅 구성원인 회원 신고 대상 |
| reason_code | TINYINT | N | - | `BLAME_TYPE_USER` 1~6 |
...
- 행사 소유 Admin과 Super Admin은 해당 미팅 상세에서 신고 목록을 조회한다. 처리는 Super Admin만 수행하며
  처리자·사유·처리 시각은 같은 transaction의 action log에만 남긴다. Mobile 기본 응답에는 CMS 처리자와
  처리 사유를 노출하지 않고 CMS 상세 DTO에서만 제공한다.
...
| actor_id | INT | N | - | MEMBER/ADMIN 내부 ID snapshot, SYSTEM은 0 |
...
| APPLICATION | `APPLICATION_APPROVED`, `APPLICATION_CANCELED`, `APPLICATION_LEFT`, `ADMIN_PROFILE_VIEWED` |
...
- 호스트 연결, 참여 승인/확정 취소, 행사 취소/삭제, 신고 처리, 메시지 삭제,
  ...
  Mobile 호스트의 행사 삭제도 서버가 고정한 사유를 기록한다. 일반 행사 생성·수정·모집·확정·자동 종료와
  `APPLICATION_LEFT`는 reason을 null로 둘 수 있으며 값이 있으면 공백이 아니어야 한다.
- request_id는 클라이언트 입력이 아니라 API 요청 컨텍스트에서 생성해 모든 action log에 저장한다.
...
| 테이블 | CHECK |
| --- | --- |
| event | `status IN (-2,-1,0,1,2,3,4) AND version > 0 AND application_close_at <= event_at AND photo_public IN (0,1) AND male_capacity >= 2 AND female_capacity >= 2 AND male_capacity + female_capacity <= 30 AND CHAR_LENGTH(TRIM(title)) > 0 AND CHAR_LENGTH(TRIM(thumbnail_path)) > 0 AND CHAR_LENGTH(TRIM(location)) > 0 AND CHAR_LENGTH(TRIM(create_idempotency_key)) > 0 AND hashtags REGEXP '^#[^[:space:]#]+( #[^[:space:]#]+)*$'` |
...
| report | `status IN (0,1,2) AND reporter_member_id > 0 AND target_member_id > 0 AND target_member_id <> reporter_member_id AND reason_code BETWEEN 1 AND 6 AND CHAR_LENGTH(TRIM(idempotency_key)) > 0 AND (reason_code <> 6 OR (content IS NOT NULL AND CHAR_LENGTH(TRIM(content)) > 0))` |
| action log 공통 | `actor_type IN (1,2,3) AND ((actor_type IN (1,2) AND actor_id > 0) OR (actor_type = 3 AND actor_id = 0)) AND target_id > 0 AND CHAR_LENGTH(TRIM(action)) > 0 AND CHAR_LENGTH(TRIM(request_id)) > 0 AND (reason IS NULL OR CHAR_LENGTH(TRIM(reason)) > 0)` |
...
chat member 생성 시 application이 실제 APPROVED인지, 보존 중에는 APPROVED/CANCELED/LEFT인지 같은 상태 조건은
...
action log 저장 전에는 action이 가리키는 application/report/message가 event_id 소속인지, attach된
...
action에 필요한 Super Admin 권한을 가져야 한다. SYSTEM actor는 `event_at + 24시간`이 지난 CONFIRMED 행사의
자동 FINISHED 전이에만 사용한다.
...
- 30일 개인정보 정리 job도 DRAFT/OPEN/CLOSED/CONFIRMED 행사의 호스트 회원은 원천 회원 row와 host/event를
  잠근 뒤 삭제 대상에서 제외하며, 행사가 종료 또는 취소된 뒤 다음 실행에서 정리한다.
...
- action actor_id는 부모 FK를 두지 않는 내부 감사 snapshot이며 부모 삭제 뒤에는 원천 회원/Admin과
  다시 연결하지 않는다.
- failed/discarded 또는 행사에서 분리된 상세 이미지 version은 원본/slice 파일을 정리하고 상태 metadata는
  polling과 감사 근거로 보존한다. 원천 Admin 계정을 삭제할 때 행사에 연결되지 않은 version/slice metadata를
  같은 transaction에서 hard delete한다.
...
| 4 | FINISHED | 모임 일시 24시간 후 자동 종료, 후기 가능 |
...
- `APPROVED -> CANCELED`는 행사 OPEN/CLOSED/CONFIRMED에서 가능한 Admin 확정 취소다. `approved_at`은 승인 증빙으로
...
- 행사 CONFIRMED 시 남아 있는 APPLIED는 상태를 변경하지 않는다. 일반 목록에서는 미확정으로 표시하되,
  기존 확정 참가자 취소 뒤 대체 인원을 받을 때 Admin이 APPLIED를 승인할 수 있다. 이 승인은 chat member
  생성과 한 transaction으로 처리한다.
- 행사 CANCELED는 신청 상태를 덮어쓰지 않는다. 행사 상태로 전체 취소를 해석한다.
- 행사 CONFIRMED를 유지한 채 application만 CANCELED로 바꾸는 것은 개인 확정 취소다. 행사 자체를
  CANCELED로 바꾸는 전체 취소와 같은 상태가 아니다.
...
3. event/application/member row를 lock하고 신청 회원이 현재 NORMAL인지 확인한 뒤 해당 성별 APPROVED 수를
   재집계한다. 참여 승인은 행사 상태가 OPEN/CLOSED/CONFIRMED이고 재집계 수가 event의 해당
   `male_capacity`/`female_capacity`보다 작을 때만 허용한다.
...
5. 확정 취소도 행사 상태가 OPEN/CLOSED/CONFIRMED일 때 application을 `APPROVED -> CANCELED`로 바꾼다.
   approved_at은 유지하고 version과 `APPLICATION_CANCELED` action log를 같은 transaction에 저장한다.
   CANCELED는 CMS에서 외부 환불이 필요한 대상으로 표시한다.
6. 모임 확정 시 호스트와 모든 APPROVED 신청 회원이 현재 NORMAL이고 남녀 APPROVED가 각각 2명 이상인지
   확인한다. 설정 정원 전체 충원은 요구하지 않으며 남은 APPLIED는 별도 상태로 바꾸지 않는다.
7. event CONFIRMED와 `host_id`를 채운 호스트, `application_id`를 채운 참가자 chat member를 한
   transaction에 저장한다. 최초 시스템 메시지는 만들지 않는다.
8. CONFIRMED 이후 신규 승인에는 해당 application의 chat member를 같은 transaction에 추가한다. 확정 취소된
   기존 참가자의 chat member는 삭제하지 않되 이후 메시지 읽기·쓰기와 새 메시지 preview를 차단한다.
9. commit 뒤 참여 승인, 확정 취소 또는 모임 확정이 실제 반영된 경우에만 각각 78, 79 또는 80 타입으로 대상별
...
1. 행사 취소는 event를 CANCELED로 바꾸고 action log를 같은 transaction에 저장한다. commit 성공 시
   ...
   저장하지 않는다.
2. 서버 cron은 `event_at + 24시간`이 지난 CONFIRMED event를 잠그고 FINISHED와 SYSTEM action log를 같은
   transaction에 저장한다. Admin 수동 종료는 허용하지 않는다. commit 성공 시 APPROVED/LEFT 참가자에게
   83 타입을 보낸다.
...
5. CONFIRMED 이후 Admin 확정 취소된 CANCELED 참가자는 채팅 목록에 종료 카드만 남기며 메시지 목록/전송/
   읽음 API를 모두 거절한다. FINISHED 행사에서 후기를 작성한 참가자는 채팅 목록에서도 제거하고 동일하게
   메시지 접근을 거절한다.
...
2. `t_setting.id=25` 값을 서버에서 읽고 작성자의 `t_member` row를 `SELECT ... FOR UPDATE`로 잠근 뒤
   기존 review를 다시 확인한다. 보상 후 잔액이 signed INT 최댓값 2147483647을 넘으면 후기·원장·잔액을
   모두 저장하지 않고 transaction을 실패시킨다.
...
5. 후기 commit 뒤 해당 참가자의 chat member row는 삭제하지 않지만 채팅 목록과 메시지 접근 자격에서 제외한다.
...
| `AdminGroupMeetingProfileViewPage` | event별 유료 profile view + 실제 Key 원장 변동량/잔액 |
| `AdminGroupMeetingReviewPage` | event별 review + 작성 application/member 표시 정보 |
| `AdminGroupMeetingReportPage` | event별 회원 신고 + CMS 전용 신고자/피신고자 표시·처리 정보 |
...
- Mobile 검색은 별도 API를 호출하지 않고 이미 내려받은 공개 행사 목록의 title, location, hashtags와
  호스트 nickname을 클라이언트에서 필터링한다. 아직 내려받지 않은 페이지까지 서버 전체 검색하는 기능은
  1차 범위에 포함하지 않는다.
- APPLIED는 내 모임에서만 “호스트 검토 중” 상태로 표시하고 채팅 목록에는 노출하지 않는다. APPROVED부터
  ...
  표시한다.
- CONFIRMED 이후 개인 확정 취소된 CANCELED 참가자는 종료 카드만 보되 unread와 마지막 메시지를 받지 않고
  메시지 API에 접근할 수 없다. 후기를 완료한 참가자는 해당 행사 채팅 카드를 더 이상 받지 않는다.
...
- 신청 row가 없는 DRAFT/OPEN/CLOSED 행사에 대한 Mobile 호스트 수정/삭제
- 신청/신청자 목록(호스트 회원)
- 호스트의 신청자 프로필 무료 열람, 승인 참가자 간 미니/사진프로필 최초 유료 열람
- 채팅방 목록, 메시지 목록/전송/읽음/퇴장
- 같은 행사 채팅 구성원 신고, 종료 후기

### Admin

- 행사 목록/상세/생성/수정/모집 시작/마감/모임 확정/취소
- 신청 목록/승인/확정 취소
- 긴 이미지 upload/status/discard
- 미팅 상세의 채팅 내역/후기 내역/회원 신고 내역/프로필 조회 결제 내역, 채팅 메시지 운영 삭제

CMS 왼쪽 대분류에는 `미팅 내역`만 둔다. 채팅·후기·신고·프로필 조회 결제 내역을 전역 대분류로 만들면
서로 다른 방의 데이터가 섞이므로 구현하지 않는다. 각 조회는 반드시
`/admin/group-meetings/{event_id}/...` 형태의 미팅 상세 하위 API로 제한한다. 전역 패널티 내역과
그룹미팅 전용 패널티 테이블/API도 1차 범위에 없다.

기존 CMS 알림 응답에는 PENDING 그룹미팅 회원 신고 수를 `groupMeetingChatUserReport`로 포함한다. 이는
벨 알림 표시와 미팅 내역 진입을 위한 집계이며 전역 신고 목록 API가 아니다. 신고를 RESOLVED/DISMISSED로
처리하면 PENDING 집계에서 제외된다.

동시 수정 대상 write API는 `version`을, 재전송 가능한 생성/메시지/신고 API는 각각 명시된 idempotency key
또는 `client_message_id`를 받는다. 상태 전이와 Admin 감사 대상 write는 API 요청 컨텍스트가 생성한
request_id를 action log에 기록하며 클라이언트가 감사용 request_id를 결정하지 않는다.

## 필수 검증

- 같은 host/create_idempotency_key와 reporter/idempotency_key 재요청이 행사/신고를 중복 생성하지 않는다.
- Mobile 검색은 이미 내려받은 목록만 필터링하며 별도 검색 API 호출을 만들지 않는다.
...
- 참여 승인과 확정 취소는 행사 OPEN/CLOSED/CONFIRMED에서만 허용한다. 확정 취소는 application을
  CANCELED로 바꾸고 approved_at을 유지하며 version과 `APPLICATION_CANCELED` action log를 함께 반영한다.
  CONFIRMED 신규 승인은 chat member 생성까지 함께 반영하고 그 밖의 행사 상태에서는 두 전이가 실패한다.
- 현재 NORMAL이 아닌 신청자는 승인할 수 없고, 호스트 또는 APPROVED 신청자가 현재 NORMAL이 아니면
  CONFIRMED 전이와 chat member 생성이 함께 실패한다.
- 동시 승인에도 행사의 `male_capacity`/`female_capacity`를 초과하지 않는다.
- 행사 생성/수정은 남녀 각각 2명 이상과 합계 30명 이하를 동시에 만족해야 한다.
- application row가 없는 DRAFT/OPEN/CLOSED는 Mobile 호스트가 수정/삭제할 수 있고, application이 한 건이라도
  생긴 뒤에는 상태와 관계없이 Mobile 수정/삭제가 실패한다.
...
- 다른 event의 application/chat member/message를 연결하거나 다른 event message를 last_read 경계로
  설정하면 복합 FK로 실패한다.
- CONFIRMED 시 남녀 승인 인원이 각각 2명 이상이며 남은 APPLIED는 미확정으로 조회된다. 이후 대체 승인을
  하면 해당 신청의 APPROVED와 chat member가 함께 반영된다.
- 호스트가 아니면서 APPROVED가 아닌 회원과 LEFT 회원은 채팅을 읽거나 쓰지 못한다.
- CONFIRMED 이후 CANCELED 참가자는 종료 카드만 받고 메시지/unread/preview를 받지 않으며, 후기 완료
  참가자는 채팅 목록과 메시지 API에서 모두 제외된다.
...
- 회원 신고는 reporter와 target이 모두 같은 행사의 현재 채팅 구성원일 때만 가능하고 target null, 본인 신고,
  행사글 신고는 실패한다. Mobile 응답에는 CMS 처리 메타데이터를 노출하지 않는다.
...
- FINISHED는 Admin 수동 전이가 아니라 `event_at + 24시간` cron에서 한 번만 반영되고 SYSTEM action log가
  남는다.
...
- CMS 전역 채팅/후기/신고/패널티 목록 API는 존재하지 않고, 미팅 상세의 event-scoped 조회만 존재한다.
...
| Gate | 적용 | 검증 |
| --- | --- | --- |
| `DBM-GATE-000` | 필수 | CHECK와 복합 UNIQUE/FK 지원, 부모 테이블 타입/collation, FCM 77~83 미사용, `t_setting.id=16/18` 존재·`-10`처럼 선행 0이 없는 signed INT 범위 음의 정수와 25 미사용 확인 |
