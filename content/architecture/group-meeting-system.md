...
- `OPEN` 이후 title, 일시, 장소, 참가비, 정원, 사진 공개 여부 변경은 신청자 영향 때문에 금지한다.
...
- 신고 목록 조회와 처리는 Super Admin만 수행하며 처리자·사유·처리 시각은 같은 transaction의 action log에만
  남긴다.
...
| actor_type | TINYINT | N | - | 1=MEMBER, 2=ADMIN |
| actor_id | INT | N | - | MEMBER/ADMIN 내부 ID snapshot |
...
- 호스트 연결, 참여 승인/확정 취소, 행사 취소/삭제, 신고 처리, 메시지 삭제,
  ...
  일반 행사 생성·수정·모집·확정·종료와 MEMBER action은 reason을 null로 둘 수 있으며 값이 있으면
  공백이 아니어야 한다.
...
| action log 공통 | `actor_type IN (1,2) AND actor_id IS NOT NULL AND actor_id > 0 AND target_id > 0 AND CHAR_LENGTH(TRIM(action)) > 0 AND CHAR_LENGTH(TRIM(request_id)) > 0 AND (reason IS NULL OR CHAR_LENGTH(TRIM(reason)) > 0)` |
...
action에 필요한 Super Admin 권한을 가져야 한다. 1차 범위의 상태 전이는 인증된 Admin 또는 MEMBER 요청으로만
기록한다.
...
3. event/application/member row를 lock하고 신청 회원이 현재 NORMAL인지 확인한 뒤 해당 성별 APPROVED 수를
   재집계한다. 참여 승인은 행사 상태가 OPEN/CLOSED이고 재집계 수가 event의 해당
...
6. 모임 확정 시 호스트와 모든 APPROVED 신청 회원이 현재 NORMAL이고 남녀 APPROVED가 각각 2명 이상인지
   확인한다. 설정 정원 전체 충원은 요구하지 않으며 남은 APPLIED는 미확정 마감으로 남긴다.
