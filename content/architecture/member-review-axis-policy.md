...
| 진행 (Progress) | 각 심사 단계의 현재 상태 | `v_member_review_status` (`basic_info_status`, `required_auth_status`, `intro_status`) | `t_member_review_status` 직접 판정 |
| 출처 (Source) | 해당 pending이 어디서 생성됐는지 | `t_member_review_item.review_origin` | 출처 없이 상태 조합만으로 큐 분류 |
...
- 소개글: `about_me`, `intro`
- 인증: `t_member_auth` 계열
- 실물 사진/동영상: `t_member_profile_version` 계열
...
- 출처 없는 레거시는 `SIGNUP_REVIEW`로 간주하고, 운영 마이그레이션으로 정리한다.
...
- 현재 출처 SoT는 `t_member_review_item.review_origin`에 완전 적용되어 있다.
- `t_member_auth`, `t_member_profile_version`은 별도 `review_origin` 컬럼이 없으므로 과도기 보조 조건이 필요하다.
