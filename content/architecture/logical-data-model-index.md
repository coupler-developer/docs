...
논리 데이터 모델은 DB 테이블을 외우기 위한 문서가 아니다. 개발할 기능의 데이터가 어디에 속하고, 무엇과
연결되며, 어떤 규칙을 지켜야 하는지 찾는 지도다.

```mermaid
flowchart LR
    Q["궁금한 데이터"] --> I["아래 표에서 업무 영역 찾기"]
    I --> O["소유 문서 열기"]
    O --> V["먼저 보는 그림 확인"]
    V --> R["구현할 때만 상세 표 확인"]
```

예를 들어 회원의 현재 상태는 `회원`, 심사 요청과 증거는 `회원 심사`, Key 잔액과 사용 기록은 `Key 지갑`이
맡는다. 같은 데이터를 여러 문서에서 다시 정의하지 않고, 아래 소유 문서 한 곳에서만 설명한다.
...
- 기존 구조를 이해할 때는 소유 문서의 `먼저 보는 그림`을 먼저 읽고, 정확한 이름과 조건이 필요할 때만 상세
  표를 확인한다.
- 새 도메인이 필요하면 이 인덱스, 소유 문서, 논리 모델 검증 fixture를 같은 변경에서 갱신한다.
- 실제 DB 구조의 이름과 연결은 비공개 서비스 저장소에서 관리한다.

## 관련 문서

- [논리 데이터 모델 정책](../policy/logical-data-model-policy.md)
- [예정 논리 데이터 모델 인덱스](logical-data-model-planned-index.md)
