...
- 영향: 현행 문서가 목표 taxonomy에 맞기 전에 hard gate를 켜면 무관 변경을 차단하거나 이관용 allowlist와 예외가 장기 SoT로 남을 수 있다.
- 조치: 구현 시점 docs 전체 baseline과 목표 taxonomy·비포함 범위를 확정하고, 대표 불일치를 작은 PR로 정리한 뒤 false-pass fixture와 report-only 검증을 먼저 추가한다.
- 완료: 목표 taxonomy 적용 대상 문서 불일치와 이관용 allowlist 0건, 잘못된 fixture 자동 실패, report-only 예상 밖 불일치 0건 확인 후 docs CI hard gate와 `yarn validate:docs` 통과.
