...
## 30) 큐레이터 `client_message_id` 호환 경로 제거 대기 `P1` `S`

- 현상: 큐레이터 실시간 채팅 호환 배포에서 기존 Admin artifact와 Mobile bundle의 전송을 깨지 않기 위해, `client_message_id`가 생략된 요청에만 API가 요청별 `compat-` 키를 생성하고 surface별 사용 로그를 남긴다. 명시적 오류 값은 거부하며 새 소비자는 키를 직접 보낸다.
- 영향: 호환 요청은 재시도 멱등성을 보장하지 않고, helper를 영구 유지하면 최종 request 계약이 optional로 남는다.
- 조치: contracts package `0.1.16` stable 발행과 Admin·Mobile exact pin → API 호환 배포 → 새 Admin 배포 → Android·iOS NextPush `Production` mandatory → 이전 bundle 강제 업데이트 차단 → `[concierge-client-message-id] compatibility fallback used` 로그를 surface별로 관측한다. 목표 시점은 mandatory 적용 직후 시작하는 24시간 연속 0건 관측이 끝난 다음 cutover PR이다.
- 완료: 24시간 연속 `ADMIN_WEB`·`MOBILE_APP` 호환 로그 0건, published latest와 세 레포 exact 계약 버전 일치, 별도 cutover PR에서 호환 helper 삭제·Swagger required 복원, 신규 요청 누락 거부 회귀 테스트와 운영 smoke 통과.

## 31) 큐레이터 offset/배열 목록 endpoint 제거 대기 `P2` `M`

- 현상: 신규 Admin·Mobile은 읽기 전용 `before_id` keyset cursor와 WebSocket 읽음 명령을 사용하지만 API 배포 시점의 기존 Admin과 Mobile bundle을 위해 `/admin/member/concierge/chat_list` 전체 배열 응답과 `/app/chat/list` offset 응답이 Deprecated 호환 endpoint로 남는다. 두 구 소비자는 별도 읽음 명령이 없으므로 해당 GET이 실제 반환한 상대방 발신 메시지 경계까지의 읽음 변경과 혼합 버전용 `read_updated` 발행도 endpoint 수명 동안 유지한다.
- 영향: 호환 endpoint를 영구 유지하면 목록 계약이 이원화되고, 기존 Admin 전체 조회 비용과 기존 Mobile offset의 실시간 append 사이 페이지 경계 이동·큰 offset 비용을 유지한다.
