...
- 영향: 프로필 열람과 환불이 겹치면 환불이 직전 조회값으로 확정되어 실제로 차감된 프로필 열람 Key가
  누락될 수 있다.
- 조치: 환불 트랜잭션이 `t_match`를 먼저 잠그고 기대 상태를 검증한 뒤 같은 connection에서 열람 여부와
  실차감 스냅샷을 읽어 회원 잔액·Key 원장·매칭 상태·매칭 로그를 함께 커밋하도록 수렴한다.
...
- 현상: 일반 유료 프로필 사진의 최초 열람 요청과 열람 상태 UPDATE가 환불 완료 상태를 거부하는 서버 조건을
  갖지 않는다.
- 영향: 환불이 끝난 카드에 직접 유료 열람 요청을 보내면 프로필 열람 상태와 Key 차감이 뒤늦게 반영될 수
  있다.
- 조치: 프로필 열람 트랜잭션에서 `t_match`를 잠그고 환불 완료 상태의 신규 열람을 명시적인 도메인 오류로
  거부한다.
