...
- `최종리뷰`, `상태 확인`처럼 대상을 생략한 짧은 후속 요청은 직전에 명시·합의한 레포·ref·파일/변경 범위와
  포함·제외 범위를 그대로 상속한다. 사용자가 새 대상을 명시하면 새 범위로 대체한다.
- 리뷰·상태 확인은 확정된 범위로 고정한다. 관련 SoT 폐쇄 탐색 문서는 판정 근거로만 열람하고 리뷰 대상에
  편입하지 않는다. 상속할 범위가 없거나 새 지시만으로 대상을 확정할 수 없으면
  범위 밖 branch/worktree/PR을 탐색하거나 추측하기 전에 사용자에게 대상 레포·ref·파일/변경 범위를 확인한다.
- 변경·branch·PR 작업에서 새 branch/worktree/PR을 만들기 전에만 같은 범위의 기존 작업 후보를 확인한다.
  적합해 보이는 후보가 있어도 자동으로 전환하지 않으며, 후보와 연관 근거를 보고하고 사용자가 계속할 작업을
  확인할 때까지 기존 작업 전환과 새 작업 생성을 모두 중지한다. 사용자가 대체 작업을 승인하면 이관·정리 계획을
  먼저 보고하고 같은 범위의 활성 PR을 병렬로 유지하지 않는다.
...
2. `CLASSIFY`: 요청 유형, 권한 집합, 상속·명시된 작업 범위, 위험 표면을 서로 분리해 고정한다.
3. `CONTINUITY`: 변경·branch·PR 작업이면 새 작업 생성 전에 같은 범위의 기존 worktree, branch, PR 후보만
   확인한다. 후보가 있으면 자동 전환하지 않고 사용자 확인을 기다린다.
4. `ROUTE`: 관련 SoT 폐쇄 탐색을 완료하고 추가 필수 문서를 읽는다.
5. `BASELINE`: 확정된 범위 안에서 현재 코드·문서·Git·외부 상태와 사용자 변경을 read-only로 확인한다.
...
- Finding이 있으면 원인을 1회 수정하고 동일 범위를 1회 독립 재리뷰한다. 재리뷰에도 열린 Finding이 남으면 최종
  판정은 `Finding`으로 유지하고 추가 수정·재리뷰 없이 자동 실행을 `BLOCKED`로 종료해 보고한다.
