...
- Store 출시와 NextPush 적용은 모바일 활성화 수단이다. source 정렬, 강제 업데이트 팝업, mandatory 설정,
  버전을 구분하지 못하는 traffic 0건은 이전 소비자의 current-API case 증빙이 아니다.
- `API cutover: Yes`여도 이전 소비자가 이해할 수 있는 bootstrap/version/업데이트 경로는 계속 성공해야
  한다. 변경된 제품 interface의 current-API case와 `expected` 선택은
...
5. API `Yes`이면 old-readable bootstrap/version 성공 case와 변경된 이전 소비자의 제품 요청 current-API case를
   검증하고, activation·client rollback이 참조할 case ID를 고정한다. activation에는 선택한 이전 소비자의
   제품 요청 case를 포함한다.
...
- API `Yes`이면 activation 순서, old-readable bootstrap/upgrade, client rollback case가 준비돼 있다.
...
3. API `Yes`이면 승인·출시 가능 상태에서 activation window 안에서 API/Admin/Store를 전환한다.
   bootstrap/version은 old-readable해야 하며 이전 소비자 제품 요청이 case의 `expected`와 일치하는지 확인한다.
...
3. API `Yes`이면 activation window 안에서 API/Admin과 양 플랫폼 label을 전환하고, old-readable
   bootstrap/upgrade와 새 계약 smoke를 확인한다.
4. Mandatory는 선택한 rollout 속성으로만 기록하고 이전 소비자 case 증빙으로 사용하지 않는다.

### 5) 완료 Gate

- 소비자 inventory의 현재·이전 case와 실제 운영 artifact가 일치한다.
- API `No`이면 모든 지원 이전 소비자가 성공하고 이번 변경이 만든 후속 공개 계약 전환 작업이 0건이다.
- API `Yes`이면 activation·이전 소비자 current-API·bootstrap/upgrade·client rollback case가 실제 순서에서 통과했다.
...
- [ ] API `Yes`이면 old-readable bootstrap/upgrade와 이전 소비자 current-API case, activation/client rollback이 있는가?
...
- 현재 source 정렬, 앱 팝업 또는 버전 미식별 traffic 0건으로 이전 소비자 case 결과를 추론하지 않는다.
