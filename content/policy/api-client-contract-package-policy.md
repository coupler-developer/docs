...
`@coupler-developer/coupler-api-contracts`는 `coupler-api`에서 생성한 API/Admin/Mobile 공통 계약의 릴리즈 스냅샷이자 public request/success DTO type 및 공통 response/error runtime 배포 경계다. API 기능 PR의 generated draft는 `main`에 누적할 수 있고, 실제 소비자 릴리즈 범위를 고정할 때 전용 contract release PR에서 한 번만 version을 올려 발행한다. Mobile/Admin은 발행된 stable package runtime으로 envelope을 검증하고, 릴리즈 스냅샷에 포함된 DTO type을 소비한다.
...
- Canonical generated contract: `coupler-api/packages/contracts/src/generated/*`에서 generator가 만든 API contract artifact. 아직 발행하지 않은 변경을 포함할 수 있는 draft이자 다음 package release의 source artifact다.
...
- Published latest stable version: contract release PR에서 범위를 고정해 GitHub Packages에 가장 최근 발행한 prerelease가 아닌 version. API `main`의 unreleased generated draft보다 뒤에 있을 수 있다.
- Unreleased contract draft: API 기능 PR에서 Swagger/OpenAPI와 함께 생성되어 `main`에 누적됐지만 아직 package version bump와 publish를 하지 않은 canonical generated contract 변경
- Contract release PR: 누적된 unreleased contract draft의 범위를 고정하고 package version을 한 번 올리는 `chore/{이름}/contracts-{버전}-발행` 브랜치 PR
...
- 계약 package의 source of truth는 `coupler-api`다. Admin/Mobile은 package를 생성하지 않고 발행된 stable version을 lockfile로 고정한다.
...
- 모든 active consumer는 published latest stable version을 `package.json`과 lockfile에 exact version으로 고정한다. API `main`의 unreleased generated draft는 이 정렬 대상이 아니며, contract release PR로 발행한 스냅샷과 Admin/Mobile version이 하나라도 다르면 해당 릴리즈의 계약 정렬과 cutover는 완료가 아니다.
- Contract release 최종 구조 리뷰에서는 발행 대상 API ref의 package source와 Admin/Mobile dependency·lockfile을 하나의 동시 배포 계약 묶음으로 비교한다. 발행 스냅샷과 두 소비자의 exact version 및 실제 runtime 공개 표면이 같으면 함께 배포 가능한 최종 계약으로 판정한다. 아직 contract release 범위에 포함하지 않은 unreleased draft는 비교 대상에서 제외하며, 이 코드 판정 자체에 Store/NextPush 이력이나 legacy traffic 운영 증빙을 요구하지 않는다.
...
- Contract release 범위가 고정되면 변경된 계약 symbol을 특정 consumer가 직접 import하지 않더라도 version 갱신 대상에서 제외하지 않는다. 계약 package는 active consumer가 함께 고정하는 공용 계약 스냅샷이다.
...
- 소비자 version 지연은 사용자가 현재 작업에서 명시적으로 승인한 별도 호환 릴리즈에서만 허용한다. 예외 기록에는 승인 근거, 대상 consumer, 현재/목표 version, 지연 사유, owner, 제거 조건과 목표 시점을 포함해야 하며, 예외가 열린 동안에는 cutover 완료로 판정하지 않는다.
...
- 발행된 package version은 재사용하지 않는다. 기능 PR에서 계약 산출물이 바뀌는 것만으로 version을 올리지 않으며, 누적 draft를 발행하는 contract release PR에서 새 version을 한 번 올리고 소비자 lockfile에 반영한다.
...
- 아직 발행하지 않은 additive endpoint를 같은 릴리즈 작업에서 개발할 때는 성공 `data`를 `unknown`으로 받고 필수 필드를 runtime 검사한 뒤 camelCase 내부 모델로 명시 변환할 수 있다. Wire 키를 그대로 가진 local DTO/generic assertion은 금지하며, Store/NextPush 릴리즈 범위를 고정할 때 contract release snapshot 발행과 package generated DTO 전환을 함께 완료한다.
...
3. 기능 PR에서는 package version을 그대로 두고 unreleased contract draft를 `main`에 누적한다.
4. 실제 소비자 릴리즈 범위를 고정할 때 `chore/{이름}/contracts-{버전}-발행` PR에서 누적 draft를 검토하고 package version을 한 번 올린다.
5. Contract release PR 병합 후 새 stable version을 발행한다.
6. Admin/Mobile은 직접 사용하는 계약 symbol 변경 여부와 무관하게 새 stable version으로 dependency와 lockfile을 함께 갱신한다.
7. 각 소비자 표준 품질 게이트와 package version/lockfile 일치를 확인한 뒤 두 소비자 PR을 `main`에 병합한다.
8. Breaking 또는 cutover 성격의 변경은 두 active consumer의 version 정렬을 포함해 API 계약 변경 모바일 릴리즈 플로우의 gate를 통과한 뒤 진행한다.
...
- [ ] 기능 PR의 contract draft가 package version을 임의로 올리지 않고, version 변경은 `chore/{이름}/contracts-{버전}-발행` PR에서만 이뤄지는가?
- [ ] Contract release snapshot의 published latest stable version과 Admin/Mobile `package.json` 및 lockfile의 exact version이 모두 같은가?
