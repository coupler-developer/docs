...
- 공통 계약 산출물: operation별 성공 `data` 타입은 `coupler-api/packages/contracts/src/generated/apiContract.ts`에서 생성하고, 공통 envelope 타입과 runtime guard는 `coupler-api/packages/contracts/src/response.ts`에 둔다. Admin/Mobile은 publish된 `@coupler-developer/coupler-api-contracts` package를 설치하고 lockfile로 고정한다.
...
- Mobile/Admin은 package response runtime으로 envelope을 검증하고 `ok`로 먼저 분기한다. 실패 세부 처리는 서버에서 생성한 error runtime contract와 [API 에러 계약 정책](api-error-contract-policy.md)을 따른다.
