...
- 재발 방지 설계: 장애·버그·회귀 수정은 증상 제거로 끝내지 않는다. 근본 원인의 재유입 경로, 자동 예방·검출 지점, 운영 관측 수단, 제거·롤백 기준을 함께 평가하고 가능한 경우 같은 PR에 실패 재현 테스트나 정적 guard를 추가한다. 자동화할 수 없으면 그 이유, 대체 수동 검증, owner, 완료 기한을 기록한다.
...
- 재발 방지 검증: 장애·버그·회귀 수정의 자동 예방·검출 수단 또는 자동화 불가 근거와 대체 검증 누락 0건
...
- 같은 원인이 다시 들어오는 경로를 식별하고, 재현 테스트·정적 검사·CI gate 중 가장 이른 차단 지점에 자동 guard를 둔다.
- 자동화가 불가능하면 대체 수동 검증, 운영 관측, owner, 완료 기한을 작업 기록에 남긴다.

**체크리스트**:

- [ ] 근본 원인 파악했는가?
- [ ] 다른 곳에도 같은 문제 있는가?
- [ ] 같은 원인의 재유입 경로와 자동 차단 지점을 확인했는가?
- [ ] 재현 테스트 또는 정적 guard를 추가했는가? 불가능하면 이유·대체 검증·owner·기한을 기록했는가?
- [ ] Edge case 테스트했는가?
- [ ] 회귀 테스트 통과했는가?
...
- 오디오 세션, 카메라, 위치, 백그라운드 모드처럼 OS 전역 상태를 바꾸는 네이티브 SDK는 모듈 import 또는 앱 시작만으로 활성화하지 않는다. 기능 사용 시작 경계에서 활성화하고 종료, 실패, 화면 이탈 경계에서 반드시 해제한다.
- `patch-package`를 포함한 서드파티 소스 패치는 기본 금지한다. 호환 버전 업그레이드/다운그레이드, 공개 확장 지점 사용, 대체 SDK 검토로 해결할 수 없는 차단 이슈에만 허용한다.
- 불가피한 서드파티 패치에는 적용 패키지의 정확한 버전, 원인과 영향 플랫폼, upstream issue/PR, 자동·수동 회귀 검증, 제거 조건, 재검증 시점, 추적 이슈를 함께 남긴다. 이 증빙이 없거나 버전 drift가 발생하면 설치/CI를 실패시킨다.
- 서드파티 SDK의 앱 키와 비밀 키는 Mobile bundle에 두지 않는다. 참가 권한이나 서명 토큰이 필요하면 API가 서버 보관 비밀 키로 최소 권한·단기 토큰을 발급한다.
