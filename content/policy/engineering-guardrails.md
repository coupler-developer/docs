...
- 코드와 문서가 같은 기준으로 안전하게 갱신되도록 SoT, 검증, 문서 역할 분리를 고정한다.
...
- 리뷰 대상 범위에서 finding이 1건이라도 있으면 완료로 간주하지 않는다.
    1. 원인 분석
    2. 코드/타입/계약 수정
    3. 레포 표준 검증셋(`test`, `typecheck`, `lint`, `format` + 정책 문서의 필수 검사) 재검증
...
- 변경 레포 기준 `test`, `typecheck`, `lint`, `format`를 검증한다.
    - 레포에서 미제공인 항목은 `N/A`로 표기하고, 미적용 근거를 PR/작업 보고에 남긴다.
...
- 기본 규칙:
    - `policy`: 규범 문서. MUST/SHOULD, SoT, 책임 경계, 금지 사항, 완료 기준을 정의한다.
    - `architecture`: 구조/모델 설명 문서. 구성요소, 데이터 흐름, 저장 구조를 설명한다.
    - `fsm`: 상태 전이 설명 문서. 상태, 전이, 예시 경로를 설명한다.
    - `flow`: 사용자/운영 시나리오 문서. 액터, 단계, 예외 흐름을 설명한다.
    - `technical-debt`: 현재 남아 있는 문제, 영향, 근거, 해결 방향, 완료 기준을 기록하는 부채 문서다. 규범 문서를 대체하지 않는다.
...
- lint/CI 통과를 merge 조건으로 둔다.
    - docs 변경 시 최소 검증은 `markdownlint`와 `mkdocs build --strict`를 모두 포함한다.
    - `content/AGENTS.md` 인덱스와 `mkdocs.yml` `nav` 동기화가 필요한 변경은 같은 PR에서 함께 반영한다.
...
- docs 변경 PR은 `pull_request` 기준으로 `markdownlint`와 `mkdocs build --strict`를 모두 통과해야 한다.
- `content/templates/` 기반 문서는 역할/문서 종류/충돌 시 우선 문서/기준 성격 메타데이터를 누락하지 않는다.
- 구조 개편, 문서 추가/이동/개명 시 `content/AGENTS.md` 인덱스와 `mkdocs.yml` `nav`를 같이 갱신한다.
- 문서 검증이 통과해도 SoT 충돌이 있으면 완료로 간주하지 않는다. 충돌은 규범 문서부터 수정한다.
...
### 테스트 변경 통제 (에이전트 작업 공통)

- 기본 원칙: 테스트 파일(`__tests__`, `*.test.*`, `__snapshots__`)은 기본적으로 수정하지 않는다.
- 테스트 변경이 필요하면 즉시 작업을 멈추고 사용자 승인 후 진행한다.
- 승인 후 테스트를 변경한 경우, 최종 보고에 변경 파일 목록과 변경 이유를 반드시 명시한다.
- 회귀 수정 시 테스트를 약화(`skip/only`, assertion 완화, 무분별한 snapshot 갱신)해서 통과시키는 행위를 금지한다.
- 워크스페이스 공용 스크립트 `docs/.github/scripts/`를 사용해 테스트 파일 잠금/해제/변경 여부를 수시 확인한다.
- 동일 사용자 권한으로 실행되는 환경에서는 기술적 강제가 100% 완전하지 않으므로, 승인 절차와 변경 이력 공개를 병행한다.
