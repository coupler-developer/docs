...
- 버그 수정 시 증상이 아닌 근본 원인을 추적하여 제거한다
- 하드코딩/Magic Number 금지: 상수로 정의한다
- 같은 문제는 같은 방식으로 해결한다 (일관성)
...
- **한 표현식에서 삼항 연산자는 1회까지만 허용한다**
  - 중첩 삼항(`a ? b : c ? d : e`) 금지
  - 분기가 2개를 넘어가면 `if / else` 또는 별도 함수로 분리
  - JSX/inline 표현에서도 동일하게 적용
  - 이유: 중첩 삼항은 가독성을 빠르게 떨어뜨리고, 조건 우선순위 오해를 만든다
