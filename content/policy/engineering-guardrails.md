...
- **스펙이 확정된 경로에서는 `?.` / `??`를 기본적으로 피한다**

  - 타입/스펙이 맞으면 "항상 존재"가 원칙이다. `?.` / `??`는 런타임 문제를 조용히 숨길 수 있다.
  - 스펙이 깨질 수 있는 케이스라면 데이터/스펙을 고정하거나, 불가능한 상태를 에러/로그/사용자 메시지로 드러내는 방식으로 해결한다
- **허용되는 경우**
...
- **lint로 강제하지 않는 이유(현재 기준)**

  - 일부 레포의 lint는 `--max-warnings 0`로 실행된다. 그래서 `warn`으로 점진 도입이 어렵다.
  - 필요해지면 "별도 실행 스크립트"로 추가 검사를 도입하는 방식이 가능하지만(예: ESLint `no-restricted-syntax`로 `??` 탐지), 기본 lint/CI를 깨지 않는 합의가 먼저 필요하다.
