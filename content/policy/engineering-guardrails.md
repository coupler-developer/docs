...
- 같은 도메인에 `policy`, `architecture`, `fsm`, `flow` 문서가 함께 존재할 수 있지만, 규범 문서는 1개만 둔다.
- 문서 메타데이터는 아래 폐쇄형 값을 사용한다.
    - `역할`: `규범`, `설명`, `시각화`, `시나리오`
    - `문서 종류`: `policy`, `architecture`, `fsm`, `flow`
