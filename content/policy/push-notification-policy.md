...
- `LOUNGE_PINNED(40)`은 목록의 전용 `/admin/lounge/pinned` 명령으로 게시글이 실제 `N → Y`로 전이할 때
  작성자에게 생성한다. 이미 `Y`인 일반 중복 요청은 새 알림 세대를 만들지 않는다. 상세
  `/admin/lounge/save`의 `pinned` 저장은 이 알림의 발생 조건이 아니다.
...
- 타입 변경은 [엔지니어링 가드레일](engineering-guardrails.md)의 API/DB 하위 호환과 contract cutover
  판정을 먼저 적용한다. Additive 타입은 직전 Mobile이 안전하게 무시할 수 있어야 하며, 기존 타입의 의미
  변경·제거는 `API cutover: Yes`로 다룬다.
...
- [ ] `LOUNGE_PINNED(40)`은 `/admin/lounge/pinned`의 실제 `N → Y`에서만 생성되고 상세 저장·해제·중복
      설정에서는 생성되지 않는가?
