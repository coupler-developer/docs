...
- 안정성 평가는 `Scope Gate -> 6개 관점 -> Finding 병합 -> Exit Gate` 순서로 수행한다.
- `Scope Gate`: 변경 유형, 대상 문서, 직접 연결 문서, 제외 범위와 `N/A` 근거를 먼저 고정한다.
- 변경 유형은 `오타/N/A`, `문서-only`, `코드+문서`, `고위험 문서-only`만 사용한다.
- 6개 관점은 아래로 고정한다.
...
- 보안/권한/결제/DB/배포/릴리즈 문서는 관련 코드 리뷰 관점을 조건부로 추가한다.
- 필수 확인 항목은 아래와 같다.
    - SoT 충돌 없음
    - 문서 역할 혼재 없음
