...
- `문서 생명주기 작업`은 `추가`, `수정`, `삭제`, `이동`, `개명`, `분리`, `통합`의 폐쇄형 값만 사용한다.
- 문서 생명주기 작업·리뷰 시에는 "이 문장이 없어도 처음 온 사람이 다음 필수 문서 또는 규범 문서까지 확실히 도달하는가"를 먼저 확인한다.
...
- `policy` 문서의 생명주기 작업을 수행하거나 정책 리뷰를 수행하면 이 Gate를 적용한다.
...
- `content/templates/docs-stability-review-template.md`의 `문서 생명주기 증빙` 절과 그 안의 판정 표는 각각
  정확히 1개 유지한다. 표 헤더는 `변경 작업 | 판정 | 근거` 3개 열로 고정하고, 모든 데이터 행도 정확히
  3개 셀을 유지한다.
- 같은 표의 첫 번째 셀에는 판정 기준 문장을 복제하지 않고 `문서 생명주기 작업` 값만 사용한다. 7개 작업마다
  독립 판정 행을 정확히 1개 유지하며, 헤더·열 수·허용 작업·누락·중복을 docs 구조 검증으로 fail-closed 확인한다.
- Markdown 표의 선행 `|` 생략은 행 의미를 바꾸지 않는다. docs 구조 검증은 이 표기 차이와 무관하게
  생명주기 증빙 절 안의 모든 표 행을 같은 계약으로 판정하며, 추가 절·표·행을 허용하지 않는다.
...
- 변경 작업은 `문서 생명주기 작업` 값만 사용하며 한 변경에 여러 작업이 있으면 모두 기록한다.
...
- Scope Gate의 변경 작업 집합과 `문서 생명주기 증빙` 표에서 `N/A`가 아닌 행의 작업 집합은 정확히 일치해야 한다.
- 이동·개명은 해당 작업 행에 `삭제`·`추가` 판정 기준의 증빙을 함께 기록한다. 이동·개명과 별개인 추가·삭제를
  실제로 수행한 경우에만 추가·삭제 행도 `N/A`가 아닌 값으로 판정한다.
- 분리·통합은 해당 작업 행에 관련 `추가`·`수정`·`삭제` 판정 기준의 증빙을 함께 기록한다. 분리·통합과 별개인
  추가·수정·삭제를 실제로 수행한 경우에만 그 작업 행도 `N/A`가 아닌 값으로 판정한다.
...
- `policy` 문서의 생명주기 작업·리뷰에는 **Policy Composition / Lifecycle Consistency Reviewer**를 조건부 필수 관점으로 적용한다.
...
- 기존 문서를 분류하는 목표 taxonomy 또는 메타데이터의 값·적용 범위를 바꾸거나, 이를 템플릿 또는
  docs 구조 검증의 새 hard gate로 추가·강화하는 변경에는
...
- 필수 확인 항목은 아래와 같다.
    ...
    - policy 문서의 생명주기 작업·리뷰에 대한 전체 정책·역방향 규범 참조 검토, 책임/우선순위, 상태·단계별 Exit Gate, 전역 절 정합성, 삭제 시 책임 승계 증빙 있음
