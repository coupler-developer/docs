...
- 릴리스 실행 기록과 기술부채 사이의 신규 참조는 가변 기술부채에서 불변 릴리스 기록으로만 둔다. 릴리스 기록은
  기술부채 문서나 fragment를 새로 링크하지 않고 당시 후속 범위와 완료 조건을 본문에 자체 완결적으로 기록한다.
...
- 과거 릴리스 기록의 역사적 사실을 taxonomy 정리를 이유로 소급 변경하거나 새 작성 계약으로 재검증하지 않는다.
  후속 설명은 이슈·장애 기록에서 추적하고, 실제 새 배포가 없는 정정용 릴리스 기록은 만들지 않는다.
  [릴리스 프로세스](release-process.md)의 게시된 nonterminal 기록 복구는 태그/Release 전에 terminal 불변조건을
  회복하는 제한된 상태 전이이며 역사적 사실 정정이나 현재 작성 계약 재해석으로 사용하지 않는다.
