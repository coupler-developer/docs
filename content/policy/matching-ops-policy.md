...
- 매칭 상태 전이, 키 차감/환불, 일정 제안 범위 검증, 클럽매니저 매칭 대상 범위, 매칭 사용자 흐름
...
- 일반 클럽매니저는 `매칭 예약` 화면의 예약목록·공통 설정 조회, 리스트 생성, 예약 확정·취소, 즉시 발송, 전체·선택 삭제를 기존과 같이 사용할 수 있다. 이번 변경은 해당 운영 권한을 매니저별로 분리하거나 축소하지 않는다.
...
- 클럽매니저 매칭 범위 전환 완료 조건: 서버가 표의 경로별 시작 회원·상대 후보 범위를 적용하고, Admin의 전담매니저 컬럼·지역×등급 필터와 대상 범위 회귀 테스트가 함께 반영된 상태다.
