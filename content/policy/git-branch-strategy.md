...
- `docs/{이름}/브랜치전략`
- `fix/{이름}/핫픽스-로그인`
- `chore/{이름}/2.0.0-릴리스-이후-정리`
- `refactor/{이름}/회원가입-유스케이스-정리`

마지막 세그먼트인 `{주제}`는 `kebab-case`로 작성하고 단어 구분자는 `-`만 사용한다.
이 기준은 신규 브랜치에 적용하며, 이미 생성된 과거 브랜치명은 소급 변경하지 않는다.

Mobile Store 제출 준비 브랜치는 `coupler-mobile-app`에서만 아래 형식을 사용한다.

- `release/{이름}/2.2.0(97)-준비`
