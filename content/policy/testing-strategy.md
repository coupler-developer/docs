...
- 장애·버그·회귀 수정은 정상 동작 검증만으로 완료하지 않는다. 같은 원인의 재유입을 가장 이른 단계에서 차단하는 실패 재현 테스트, 정적 검사, CI gate 중 하나 이상을 같은 PR에 추가한다.
- 자동 재발 방지가 불가능하면 불가능한 이유, 대체 수동 검증, 운영 관측 수단, owner, 완료 기한을 PR에 기록한다.
