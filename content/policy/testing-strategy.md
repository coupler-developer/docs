...
- `coupler-mobile-app`에서 입력 필드 포커스, 키보드 노출, 터치 이벤트를 iOS Simulator로 검증할 때는 소프트웨어 키보드 표시 여부와 하드웨어 키보드 연결 여부를 검증 기록에 남긴다.
- 소프트웨어 키보드가 숨겨진 상태에서는 입력 필드를 먼저 터치한 뒤 기대 동작을 확인한다.
- `Command + K`는 소프트웨어 키보드 표시/숨김 토글로 사용한다.
- 하드웨어 키보드 연결이 켜져 있으면 입력이 Mac 키보드 경로로 처리될 수 있으므로, 터치 기반 포커스/키보드 노출 재현 결과를 실제 기기 결과와 동일하게 간주하지 않는다.
