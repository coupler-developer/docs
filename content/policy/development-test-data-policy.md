...
- N:N 그룹미팅 프로필 열람 데이터의 열람자와 대상은 같은 행사의 서로 다른 승인 참여자여야 한다. 개발 데이터 verifier는 Mobile 프로필 열람 API와 동일하게 양쪽 신청 상태와 자기 자신 조회 금지를 검증한다.
...
- [ ] N:N 그룹미팅의 상태·취소 진입·신청·메시지·신고·후기·프로필 공개·상세 이미지 obligation, 기준 참여자와 프로필 열람 양쪽의 승인 신청이 일치하는가?
