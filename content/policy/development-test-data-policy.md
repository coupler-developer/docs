...
- N:N 그룹미팅의 참여 가능한 모든 행사에는 `dummy-female@coupler.dev` 신청과 채팅 멤버십이 도메인 상태에 맞게 존재해야 한다. 초안·삭제처럼 신청을 허용하지 않는 상태에는 참여 행을 만들지 않는다.
...
- [ ] N:N 그룹미팅의 상태·취소 진입·신청·메시지·신고·후기·프로필 공개·상세 이미지 obligation과 기준 참여자가 일치하는가?
