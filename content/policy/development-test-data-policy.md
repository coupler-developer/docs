...
- N:N 그룹미팅은 저장 행사 상태 전체와 기준 시각의 실효 상태, 취소 진입 상태, 신청 상태, 시스템 메시지, 신고, 후기, 프로필 공개와 상세 이미지 처리 상태를 각각 exhaustive obligation으로 검증한다. 행사와 신청의 `version`은 기록된 상태 전이 횟수보다 정확히 1 커야 한다. 후기 완료로 `APPROVED -> LEFT`가 된 신청은 `review_completed` action을 가지되 `PARTICIPANT_LEFT` 시스템 메시지를 만들지 않아야 한다.
...
- N:N 그룹미팅 채팅은 전날 오후 1시 개방 전 `CONFIRMED`, 개방 후 `CONFIRMED`, 저장은 `CONFIRMED`지만 시작 +24시간이 지난 실효 `FINISHED`, 저장 `FINISHED`, 확정 후 `CANCELED`를 각각 둔다. 개방 후 방과 세 읽기 전용 방에는 두 기준 회원의 일반 메시지를 두고, 활성 확정 방에는 호스트 메시지와 관리자 삭제 메시지도 둔다. verifier는 운영과 같은 파생식에 행사별 개방·종료 경계의 1초 전·정각·1초 후를 대입해 저장·실효 상태와 쓰기 가능 여부를 확인한다. 행사 생성·상세 이미지 처리·상태 전이·신청·승인·퇴장/취소·채팅·후기·신고와 각 action은 허용 상태의 인과 순서와 기준 시각 이전을 지켜야 하며, 신청·승인은 신청 마감보다 앞서야 한다. 두 기준 회원의 읽음 위치는 같은 행사의 메시지를 가리키며 호스트의 읽음 위치는 비어 있는 분기를 검증한다. Admin read model은 발신자 역할, 삭제 tombstone, 시스템 메시지를 모두 반환해야 한다.
