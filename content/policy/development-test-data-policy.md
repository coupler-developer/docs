...
- N:N 그룹미팅 채팅의 공개 프로필은 채팅이 초기화된 활성 OPEN·CLOSED·CONFIRMED와 FINISHED 행사에서 호스트와 승인 참여자에게 무료로 제공하며 본인 조회도 허용한다. 결제·Key 시스템은 현행이지만 이번 N:N 프로필 조회에는 연결하지 않는다. `group-meeting-all`과 `cms-all`은 N:N 프로필 열람 행 또는 Key 차감 원장을 만들지 않고 verifier가 두 저장 건수 0을 검증한다. 별도 과금 요구가 승인되기 전에는 N:N 그룹미팅 과금 fixture를 추가하지 않는다.
