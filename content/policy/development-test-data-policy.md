...
- N:N 그룹미팅의 무료 참가자 프로필 fixture는 채팅이 초기화된 실효 `OPEN`·`CONFIRMED` 행사에서 호스트와 승인 참여자에게 제공하며 본인 조회도 허용한다. `FINISHED`·`CANCELED`와 행사 시작 +24시간 경계를 지난 방은 채팅 이력·후기 fixture만 유지하고 전체 프로필·카드 동작 fixture에서 제외한다. 결제·Key 시스템은 현행이지만 이번 N:N 프로필 조회에는 연결하지 않는다. `group-meeting-all`과 `cms-all`은 N:N 프로필 열람 행 또는 Key 차감 원장을 만들지 않고 verifier가 두 저장 건수 0을 검증한다. 별도 과금 요구가 승인되기 전에는 N:N 그룹미팅 과금 fixture를 추가하지 않는다.
...
| `lounge-all` | 카테고리, 정상·고정글·삭제 글, 댓글·대댓글·삭제 tombstone, 좋아요, 신고, 회원 차단, 패널티 |
...
- 테스트 데이터 insert·update·delete는 서비스 DB migration source에 포함하지 않는다.
...
- [ ] N:N 그룹미팅의 저장·실효 상태·취소 진입, 채팅 개방 전·후·종료 경계, 기본 기준 여성 회원 2명의 신청과
  ...
  obligation이 일치하고, Admin 행사·채팅·신고·패널티 목록에 노출되며 실효 활성 행사만 포함한 현재 무료
  프로필 조회의 저장 행·Key 차감 원장이 0건인가?
