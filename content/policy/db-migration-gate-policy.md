...
| `DBM-GATE-010` | Migration Ledger | 수동 SQL을 공유/장기 보존 DB에 반영할 때 | 적용 전 중복/체크섬 검증, 적용 후 DB-local 성공 이력 기록 | `schema_migrations` 조회/insert 결과 또는 동등한 migration tool ledger, 실제 DB 식별값 | 즉시 중단 |
...
- 공유/장기 보존 DB에 적용하는 각 SQL 파일은 성공 후 `schema_migrations`에 기록한다. postcheck-only SQL도 실행 순서에 포함되면 동일하게 기록한다.
...
- 별도 migration tool이 없는 수동 SQL 운영에서는 공유/장기 보존 DB마다 `schema_migrations`를 둔다.
- `schema_migrations` 또는 동등한 tool ledger가 아직 없으면 `DBM-GATE-010`은 `pending-debt`로 표기하고, 기술부채 항목과 수동 증빙 로그를 함께 남긴다.
...
- `schema_migrations`가 없는 DB에는 비즈니스 마이그레이션보다 먼저 이력 테이블 부트스트랩 SQL만 단독 적용한다.
- 부트스트랩 SQL은 `migration_type='bootstrap'`으로 자기 자신을 기록하고, 이후 모든 SQL은 이 테이블을 기준으로 중복/체크섬을 판정한다.
...
- `DBM-GATE-010` 적용 대상 DB에서 적용 완료 row 또는 동등한 migration tool ledger가 없으면 ledger 통제 완료로 판정하지 않는다. 단, ledger 미도입 기술부채가 열려 있는 동안에는 `DBM-GATE-010 pending-debt`와 수동 증빙으로 schema/data safety gate와 분리해 기록한다.
...
2. `이력 테이블 부트스트랩`: ledger 도입 작업을 수행하는 경우, 공유/장기 보존 DB에 부트스트랩 SQL만 먼저 적용하고 자기 자신을 `migration_type='bootstrap'`으로 기록한다. 다른 비즈니스 마이그레이션과 묶어 실행하지 않는다.
3. `이력 확인`: 대상 DB의 `schema_migrations` 또는 동등한 migration tool ledger를 조회해 적용 예정 SQL의 중복/체크섬 불일치가 없는지 확인한다. ledger 미도입 상태에서는 `DBM-GATE-010 pending-debt`와 수동 증빙 로그를 남긴다.
4. `Local 마이그레이션 검증`: 운영 `dump` 기준 로컬 DB에 신규 DDL/backfill/cutover/contract SQL을 순서대로 실행하고, `DBM-GATE-100/200/300/400` 통과 전까지 수정-재실행을 반복한다.
5. `Local 적용 이력 기록`: 장기 보존 local DB에 반영한 경우 각 SQL과 postcheck 성공 후 `schema_migrations`에 `target_env='local'` 및 실제 DB 식별값을 기록한다. 일회성 로컬 검증 DB는 검증 로그로 대체할 수 있다.
6. `개발계 사전 검증`: 로컬에서 통과한 동일 SQL 세트를 개발계 DB에 동일 순서로 적용하고, 카운터/해시/guard 결과가 동일 결론(`No Findings`)인지 확인한 뒤 `target_env='dev'`로 기록한다.
7. `운영계 반영`: Local+개발계 검증 완료 후에만 운영계 MySQL에 동일 SQL 세트를 반영한다. 운영계 반영 실패 시 Local 단계부터 다시 수행한다.
8. `운영계 postcheck`: 운영계 반영 직후 동일 Gate 기준 postcheck SQL을 실행해 최종 guard 결과를 확인한다. ledger가 있으면 성공 후 `target_env='prod'`로 기록하고, 없으면 `DBM-GATE-010 pending-debt`와 수동 증빙을 남긴다.
9. `근거 기록`: PR/리뷰에 적용 대상 Gate별 `Gate ID + SQL 파일 경로 + 로그 경로 + schema_migrations row 또는 migration tool ledger`를 남기고, 비적용 Gate는 `Gate ID + N/A 사유 + 근거 경로`를 남긴다.
10. `승인 조건`: Local/개발계 DB 검증 근거가 없으면 승인하지 않는다. 운영 반영 완료 판정은 운영계 postcheck 로그와 ledger 기록을 함께 확인하는 것을 목표로 하며, ledger 미도입 상태에서는 `DBM-GATE-010 pending-debt`와 수동 증빙을 별도 기록한다.
