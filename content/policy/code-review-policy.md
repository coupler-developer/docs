...
- 모든 리뷰 코멘트(MUST/SHOULD/SUGGEST/QUESTION)는 아래 중 최소 1개 근거를 포함한다.
  ...
  - 코드 라인 링크 (예: `coupler-api/src/foo.ts:42`)
...
- [ ] 코드가 읽기 쉽고 이해하기 쉬운가?
- [ ] 중복 코드는 없는가?
- [ ] [엔지니어링 가드레일](engineering-guardrails.md) 준수 여부
- [ ] 레이어 책임 분리(UI/Domain/Infra) 경계를 넘는 로직 침범이 없는가? (`engineering-guardrails.md`의 `레이어 책임 분리 (단일 SoT)` 기준)
...
- [ ] 로직 교체 시 Shadow Cutover 규칙(병렬 계산/diff 로그/불일치 0 확인/점진 교체/최종 레거시 제거)을 지켰는가?
- [ ] 불일치 0건 근거(검증 범위, 기간/샘플 수, 로그 링크)가 PR에 명시되어 있는가?
- [ ] 파생 호환 로직에 제거 조건, 담당자, 목표 시점, 추적 이슈가 명시되어 있는가?
- [ ] 제거 변경이 있을 때 삭제 대상 지정 근거(문서/이슈/승인)가 PR에 명시되어 있는가?
