...
- 일반 코드 변경은 최소 1명 이상의 비작성자 승인을 받는다.
- Store version·build 값과 그 정합성 테스트만 바꾸는 Mobile release PR은 필수 CI 통과 후 저장소 책임자가
  직접 병합할 수 있으며 별도 GitHub Review 승인을 요구하지 않는다.
...
- 구조/재사용 판정: 의미와 계약이 같은 기존 공통 component/helper/query/contract 재사용 여부, 비재사용 또는 새 공통화 근거, 과도한 추상화 N/A 근거
...
- DB 검증: append-only migration, Docker MySQL·MariaDB 결과, 개발·운영 마이그레이션 소스 커밋과 pending
  결과 또는 `N/A` 근거
...
- 모든 코드/기능 리뷰는 [엔지니어링 가드레일](engineering-guardrails.md)의 `구조 단순화 우선` 원칙을 함께 적용해, 변경 범위 안에서 더 단순한 문서/코드 구조, SoT, 책임 경계, 파일 배치와 의미·계약이 같은 기존 공통 구조의 재사용이 가능한지 확인한다. 재사용이 서로 다른 use-case를 결합하거나 추상화를 늘리면 강제하지 않는다. 변경 범위 밖 전면 리팩터링은 finding으로 강제하지 않고 `기존 부채` 또는 근거 있는 제안으로 분리한다.
...
- **Senior Frontend / Client**: Mobile/Admin UI 상태, API 호출 경계, 실패 응답 분기 기준, 로컬 상태와 서버 상태 혼용, 클라이언트 로컬 subset과 서버 분류 체계(taxonomy)의 충돌 여부, 디자인 토큰, React Native `StyleSheet.create` 신규 key의 `lowerCamelCase` 준수를 확인한다.
  Android 변경은 [R8 호환성 기준](engineering-guardrails.md#android-r8)의 적용 여부와 보존 규칙의
  범위·근거, [R8 검증 증빙](testing-strategy.md#android-r8)을 확인한다.
...
공통 완료 증빙은 위 `작성자용 PR 본문 템플릿`의 체크리스트와 변경 신호에 해당하는 단일 SoT에서
판정한다. 아래에는 리뷰어 고유 질문만 둔다.

- [ ] 요구사항과 고정된 리뷰 범위를 정확히 평가했는가?
- [ ] 작성자가 제시한 코드·문서·테스트·로그 근거가 실제 최종 후보와 일치하는가?
- [ ] 코드/기능 변경의 7개 관점과 조건부 도메인 관점에 누락 또는 근거 없는 `N/A`가 없는가?
- [ ] 이번 변경이 새로 만들거나 확산한 문제와 범위 밖 기존 부채를 구분했는가?
- [ ] 의미·계약이 같은 기존 공통 구조를 재사용할 수 있는데도 중복·우회·임시 구조를 만들지 않았고, 서로 다른 use-case를 억지로 공통화하지 않았는가?
- [ ] 코드/기능 변경이면 [엔지니어링 가드레일](engineering-guardrails.md)의 예방 통제 baseline 증빙이 실제 통제 경로·사용자 계획 결정과 일치하고 미해결 결정이 없는가?
- [ ] 회귀, 보안, 성능과 외부 의존성 위험이 적용 정책의 Gate로 검증됐는가?
- [ ] API·DB·권한·결제·릴리스 등 변경 신호를 해당 단일 SoT와 체크리스트로 라우팅했는가?
- [ ] 다중 레포 계약의 최종 상태와 병합 순서가 branch-local 사실을 최종 상태로 오인하지 않는가?
- [ ] 문서 변경이 기존 규범과 충돌하지 않고 처음 온 독자가 다음 필수 문서에 도달할 수 있는가?
- [ ] 최종 리뷰가 같은 판단·근거·해결안을 반복하지 않고 Finding 중심으로 간결한가?
- [ ] 마지막 변경 이후 열린 Finding 0건 체크포인트와 동일 후보의 검증 상태가 일치하는가?
...
- API 변경의 기본 전제는 release-scoped 지원 이전 소비자가 현재 API+최종 DB에서 성공하는
  `하위 호환 변경`이다. Store 출시와 NextPush는 별도 모바일 활성화 범위다. DB 변경은 append-only source와
  적용 순서를 별도로 검증한다.
...
공통 완료 항목은 위 `작성자용 PR 본문 템플릿` 체크리스트를 사용하고, 병합 직전에는 아래만 다시 확인한다.

- [ ] 모든 리뷰 코멘트가 해결되거나 근거 있는 후속 작업으로 분리됐는가?
- [ ] 변경 유형에 필요한 승인 또는 Mobile release PR 직접 병합 조건을 충족했는가?
- [ ] 동일 최종 후보의 필수 CI가 통과했는가?
- [ ] 충돌 해결 뒤 리뷰·검증이 만료되지 않았는가?
