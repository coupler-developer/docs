...
- Finding이 있으면 원인을 수정한 뒤 같은 범위의 독립 리뷰를 다시 수행한다. 열린 Finding이 남아 있는 동안에는
  [테스트/CI 전략](testing-strategy.md)의 로컬 표준 통합 품질 게이트를 실행하지 않는다.
...
- Finding이 있으면 `원인 수정 -> 동일 범위 독립 재리뷰`를 반복하고, 열린 Finding이 0건인 동일 최종 후보에만
  적용 품질 게이트를 실행한다. 검증 실패로 파일을 수정하면 새 최종 후보로 독립 리뷰부터 다시 시작한다.
...
- [ ] 외부 의존성 추가·대체가 있다면 표준 라이브러리·기존 직접 의존성 검토, 대안, exact package 범위와 명시적 승인 근거가 있는가?
