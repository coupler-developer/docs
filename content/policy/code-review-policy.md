...
- 코드 리뷰, 원인 조사, 영향 분석처럼 사용자가 `리뷰`, `조사`, `확인`을 요청한 경우, 명시적인 수정 요청 전에는 기존 작업 브랜치 파일을 수정하지 않는다.
- 사용자가 `git worktree`, `새 worktree`, `worktree 파서`처럼 worktree 생성을 명시하면 대상 레포마다 별도 `git worktree`를 생성해 진행한다.
- 사용자가 현재 브랜치, 특정 PR, 특정 커밋을 최신화한 뒤 리뷰를 요청한 경우에는 해당 checkout을 그대로 읽기 중심으로 리뷰할 수 있다.
- 재현용 변경, 수정 후보 검증, 테스트 추가처럼 파일 변경이 필요하면 대상 레포마다 별도 `git worktree`를 우선 생성해 원본 작업 브랜치와 분리한다.
- 별도 worktree를 만들면 작업 시작 전에 worktree 경로와 브랜치명을 확인하고, 최종 보고에도 같은 경로와 브랜치명을 남긴다.
- 격리 worktree의 변경은 `수정 후보` 또는 `검증용 변경`으로 구분한다. 사용자가 반영을 승인하기 전에는 원본 레포/브랜치로 옮기지 않는다.
- 조사 후 사용자가 수정을 승인하면 반영 목적의 정식 브랜치/워크트리로 분리하거나 변경 성격에 맞는 브랜치명으로 전환한 뒤 진행한다.
- 원본 레포에 미커밋 변경이 있으면 사용자 작업으로 보고 보호한다. 조사 범위와 충돌하면 원본 변경을 되돌리지 말고 별도 worktree에서 재현하거나 충돌 사실을 보고한다.
