...
기록이 없으면 릴리스 기록 템플릿, lifecycle registry, `content/AGENTS.md` 인덱스와 `mkdocs.yml` nav를 한 번에
초기화한다. 생성된 `planned`는 로컬 초안이며 push하지 않는다. 범위·기준 SHA·검증·rollback 기준을 작성해
`pending`으로 바꾼 첫 후보만 Draft PR에 push한다. 기록이 있으면 현재 PR·CI·preflight 상태를 확인하고 다음
scope를 안내하며, commit·push·PR·배포를 대신 실행하지 않는다.
